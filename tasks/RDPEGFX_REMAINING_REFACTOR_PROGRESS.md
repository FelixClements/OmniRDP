...
15. `US-015` — blocked/pending — Automated validation evidence is recorded in `tasks/RDPEGFX_US015_VALIDATION_EVIDENCE.md`, but required real-client/runtime validation was not run because no MSTSC/FreeRDP viewer session, backend Windows RDP server, multi-viewer lab, or controlled slow/no-ACK environment was available. Keep US-015 pending until runtime evidence is added.
