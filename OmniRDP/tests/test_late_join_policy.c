...
static void test_caps_selection_prefers_highest_version_without_canonical_caps(void)
{
    const RDPGFX_CAPSET advertised[] = {
        ...
        { .version = RDPGFX_CAPVERSION_107, .flags = RDPGFX_CAPS_FLAG_AVC420_ENABLED },
        { .version = RDPGFX_CAPVERSION_103, .flags = RDPGFX_CAPS_FLAG_SMALL_CACHE },
    };
    RDPGFX_CAPSET selected = { 0 };

    assert(viewer_gfx_select_compatible_caps(NULL, FALSE, advertised,
                                             (UINT16)(sizeof(advertised) / sizeof(advertised[0])),
                                             &selected));
...
static void test_caps_selection_requires_exact_canonical_match(void)
{
    ...
    assert(viewer_gfx_select_compatible_caps(&canonical, TRUE, compatible,
                                             (UINT16)(sizeof(compatible) / sizeof(compatible[0])),
                                             &selected));
    assert(selected.version == canonical.version);
    assert(selected.flags == canonical.flags);

    assert(!viewer_gfx_select_compatible_caps(&canonical, TRUE, incompatible,
                                              (UINT16)(sizeof(incompatible) / sizeof(incompatible[0])),
                                              &selected));
}
...
static void test_joined_state_controls_drdynvc_init_and_rdpgfx_open_paths(void)
{
    ViewerGraphicsContext gfx = { 0 };
...
{
    assert(!viewer_late_join_timeout_fallback_due(VIEWER_JOIN_STRATEGY_CLASSIC_FALLBACK,
                                                  1000, 5000, 3000, TRUE));
    assert(!viewer_late_join_timeout_fallback_due(VIEWER_JOIN_STRATEGY_REPLAY_SAFE_FRAME,
                                                  1000, 3999, 3000, TRUE));
    assert(viewer_late_join_timeout_fallback_due(VIEWER_JOIN_STRATEGY_REPLAY_SAFE_FRAME,
                                                 1000, 4000, 3000, TRUE));
    assert(!viewer_late_join_timeout_fallback_due(VIEWER_JOIN_STRATEGY_REPLAY_SAFE_FRAME,
                                                  1000, 5000, 3000, FALSE));
    assert(viewer_late_join_timeout_fallback_due(VIEWER_JOIN_STRATEGY_WAIT_NEXT_SAFE_FRAME,
                                                  2000, 5000, 3000, FALSE));
    assert(!viewer_late_join_timeout_fallback_due(VIEWER_JOIN_STRATEGY_WAIT_NEXT_SAFE_FRAME,
                                                   2000, 1999, 3000, FALSE));
}

static void test_activation_waits_for_caps_and_caps_confirm_can_restart_join(void)
{
    ViewerGraphicsContext gfx = { 0 };
