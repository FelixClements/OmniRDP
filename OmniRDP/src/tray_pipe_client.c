...
pipe_client_init(PipeClient *client, const char *pipeName)
{
    ...
    _snprintf(client->pipeName, sizeof(client->pipeName),
              "\\\\.\\pipe\\%s", pipeName);
...
pipe_client_connect(PipeClient *client)
{
    ...
    if (hPipe == INVALID_HANDLE_VALUE)
    {
        ...
        LOG_W(LOG_TAG, "Failed to connect to %s (error %lu)",
              client->pipeName, err);
        return -1;
    }
...
int
pipe_client_send_request(PipeClient *client, const PipeRequest *request,
                         PipeResponse *response, DWORD timeoutMs)
{
     ...
     */
    {
         ...
         */
        size_t needed = 64
                      + strlen(request->instance_name)
                      + strlen(request->json_payload);
    ...
    /* ── Parse the response JSON into PipeResponse ──────────────── */
    {
        ...
        if (p)
        {
            ...
            strncpy(response->json_payload, p,
                    sizeof(response->json_payload) - 1);
...
int
pipe_client_recv_push(PipeClient *client, PipePushType *pushType,
