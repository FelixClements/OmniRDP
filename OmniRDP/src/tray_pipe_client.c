...
void
pipe_client_init(PipeClient* client, const char* pipeName)
{
	if (!client || !pipeName)
		return;
	...
	_snprintf(client->pipeName, sizeof(client->pipeName), "\\\\.\\pipe\\%s", pipeName);
...
int
pipe_client_connect(PipeClient* client)
{
	if (!client)
		return -1;
	...
	HANDLE hPipe =
	    CreateFileA(client->pipeName, GENERIC_READ | GENERIC_WRITE,
	...
	if (hPipe == INVALID_HANDLE_VALUE)
	{
		...
		LOG_W(LOG_TAG, "Failed to connect to %s (error %lu)", client->pipeName, err);
		return -1;
	}
...
void
pipe_client_disconnect(PipeClient* client)
{
	if (!client)
		return;
...
BOOL
pipe_client_is_connected(PipeClient* client)
{
	return (client && client->connected) ? TRUE : FALSE;
}

int
pipe_client_send_request(PipeClient* client, const PipeRequest* request, PipeResponse* response,
                         DWORD timeoutMs)
{
	char* jsonReq = NULL;
	char* reply = NULL;
	 ...
	 */
	{
		 ...
		 */
		size_t needed = 64 + strlen(request->instance_name) + strlen(request->json_payload);
		...
		_snprintf(jsonReq, needed, "{\"cmd\":%d,\"instance_name\":\"%s\",\"payload\":%s}",
	...
	/* ── Send the frame ─────────────────────────────────────────── */
	{
		...
		LOG_D("pipe_client", "send_request: pipe_frame_send returned %d, lastError=%lu", sendResult,
		      GetLastError());
		if (!sendResult)
		{
			DWORD err = GetLastError();
...
int
pipe_client_recv_push(PipeClient* client, PipePushType* pushType, char** payload, DWORD* payloadLen)
{
	char* msg = NULL;
	DWORD msgLen = 0;
	DWORD avail = 0;
	...
	*payloadLen = 0;
	...
	if (!PeekNamedPipe(client->hPipe, NULL, 0, NULL, &avail, NULL) || avail == 0)
	{
		LeaveCriticalSection(&client->lock);
...
void
pipe_client_cleanup(PipeClient* client)
{
	if (!client)
		return;
