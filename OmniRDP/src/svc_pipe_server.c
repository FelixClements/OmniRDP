...
static int extract_instance_name(const char *payload,
                                 char *name, size_t nameSize);

/* Command handlers */
static void cmd_list_instances(PipeServer *server,
...
static void cmd_restart_instance(PipeServer *server, const char *payload,
                                 char *response, size_t respSize);
static void cmd_reload_config(PipeServer *server,
                              char *response, size_t respSize);
static void cmd_get_logs(PipeServer *server, const char *payload,
                         char *response, size_t respSize);

/* Push helpers */
static BOOL push_write_frame(HANDLE hPipe,
                             const char *json, DWORD jsonLen);
 ...
 */
static BOOL
build_pipe_security_attributes(SECURITY_ATTRIBUTES *sa,
                               SECURITY_DESCRIPTOR *sd,
                               PACL *aclOut)
{
    /*
        ...
        if (WTSQueryUserToken(WTSGetActiveConsoleSessionId(), &hToken)) {
            if (GetTokenInformation(hToken, TokenUser, NULL, 0, &tokenInfoLen)) {
                ...
                if (pTokenUser && GetTokenInformation(hToken, TokenUser, pTokenUser, tokenInfoLen, &tokenInfoLen)) {
                    AddAccessAllowedAceEx(acl, ACL_REVISION, 0,
                                          GENERIC_READ | GENERIC_WRITE, pTokenUser->User.Sid);
                }
            ...
            PSID sid = NULL;
            if (AllocateAndInitializeSid(&ntAuth, 1, SECURITY_INTERACTIVE_RID,
                                          0, 0, 0, 0, 0, 0, 0, &sid)) {
                AddAccessAllowedAceEx(acl, ACL_REVISION, 0,
                                      GENERIC_READ | GENERIC_WRITE, sid);
                FreeSid(sid);
...
json_escape_string(const char *input, char *output, size_t outSize)
{
    ...
    for (const char *p = input; *p && j < outSize - 1; p++) {
        ...
        switch (c) {
            case '"':  if (j + 2 < outSize) { output[j++] = '\\'; output[j++] = '"';  } break;
            case '\\': if (j + 2 < outSize) { output[j++] = '\\'; output[j++] = '\\'; } break;
            case '\n': if (j + 2 < outSize) { output[j++] = '\\'; output[j++] = 'n';  } break;
            case '\r': if (j + 2 < outSize) { output[j++] = '\\'; output[j++] = 'r';  } break;
            case '\t': if (j + 2 < outSize) { output[j++] = '\\'; output[j++] = 't';  } break;
            default:
                if (c >= 0x20) {
                    if (j < outSize - 1) output[j++] = (char)c;
                }
                break;
        }
    }
    ...
    while (server->running) {
        ...
        switch (cmd) {
            ...
            case PIPE_CMD_START_INSTANCE:
                cmd_start_instance(server, payload,
                                   response, sizeof(response));
                break;

            case PIPE_CMD_STOP_INSTANCE:
                cmd_stop_instance(server, payload,
                                  response, sizeof(response));
                break;

            case PIPE_CMD_RESTART_INSTANCE:
                cmd_restart_instance(server, payload,
                                     response, sizeof(response));
                break;
...
listener_thread(LPVOID arg)
{
    ...
    while (server->running) {
        ...
        ClientConnection *client = (ClientConnection *)
            calloc(1, sizeof(ClientConnection));
        if (!client) {
        ...
        HANDLE hThread = CreateThread(NULL, 0, client_handler,
                                      client, 0, NULL);
        if (!hThread) {
...
cmd_list_instances(PipeServer *server, char *response, size_t respSize)
{
    ...
    for (unsigned int i = 0; i < server->mgr->instanceCount; i++) {
        ...
        int written = _snprintf(array + pos, sizeof(array) - pos,
            ...
            (int)info.state,
            info.viewer_count,
            info.backend_hostname,
            (unsigned int)info.backend_port);
    ...
    _snprintf(response, respSize,
              ...
              "\"error_message\":\"\","
              "\"json_payload\":\"{\\\"instances\\\":[%s]}\"}", escaped);
}

/**
 * @brief Handle PIPE_CMD_START_INSTANCE.
 *
 * Response: {"type":"response","success":1,"error_message":"","json_payload":""}
 *          or {"type":"response","success":0,"error_message":"...","json_payload":""}
 */
static void
cmd_start_instance(PipeServer *server, const char *payload,
                   char *response, size_t respSize)
{
    if (!server->mgr) {
        _snprintf(response, respSize,
 ...
 */
static void
cmd_stop_instance(PipeServer *server, const char *payload,
                  char *response, size_t respSize)
{
    if (!server->mgr) {
        _snprintf(response, respSize,
 ...
 */
static void
cmd_restart_instance(PipeServer *server, const char *payload,
                     char *response, size_t respSize)
{
    if (!server->mgr) {
        _snprintf(response, respSize,
 ...
 */
static void
cmd_get_logs(PipeServer *server, const char *payload,
             char *response, size_t respSize)
{
    ...
    while (readPos > 0 && lineCount < MAX_LOG_LINES) {
        long chunkSize = (readPos >= (long)TAIL_CHUNK_SIZE)
                             ? (long)TAIL_CHUNK_SIZE
                             : readPos;
        readPos -= chunkSize;
    ...
    while (line && *line && outPos < sizeof(linesBuf) - 64) {
        ...
        for (const char *src = line; *src && outPos < sizeof(linesBuf) - 8; src++) {
            char c = *src;
            switch (c) {
                case '"':  linesBuf[outPos++] = '\\'; linesBuf[outPos++] = '"';  break;
                case '\\': linesBuf[outPos++] = '\\'; linesBuf[outPos++] = '\\'; break;
                case '\t': linesBuf[outPos++] = '\\'; linesBuf[outPos++] = 't';  break;
                case '\r': /* skip */ break;
                case '\n': /* skip */ break;
                default:
...
int
pipe_server_init(PipeServer *server, const char *pipeName,
                 InstanceManager *mgr)
{
    ...
    _snprintf(server->pipeName, sizeof(server->pipeName),
              "\\\\.\\pipe\\%s", pipeName);
    ...
    server->hListenerThread = CreateThread(NULL, 0, listener_thread,
                                           server, 0, NULL);
...
pipe_server_stop(PipeServer *server)
{
    ...
    if (server->hListenerThread) {
        if (WaitForSingleObject(server->hListenerThread, 5000)
                == WAIT_TIMEOUT) {
    ...
    while (curr) {
        ...
        if (curr->hThread) {
            if (WaitForSingleObject(curr->hThread, 1000)
                    == WAIT_TIMEOUT) {
                LOG_W("pipe_server",
                      "Client thread %p did not exit within 1s",
                      (void *)curr->hThread);
            }
            CloseHandle(curr->hThread);
            curr->hThread = NULL;
...
void
pipe_server_push_event(PipeServer *server, const char *eventType,
