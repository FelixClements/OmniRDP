...
static BOOL build_pipe_security_attributes(SECURITY_ATTRIBUTES* sa, SECURITY_DESCRIPTOR* sd,
                                           PACL* aclOut);
...
static int extract_instance_name(const char* payload, char* name, size_t nameSize);

/* Command handlers */
static void cmd_list_instances(PipeServer* server, char* response, size_t respSize);
static void cmd_start_instance(PipeServer* server, const char* payload, char* response,
                               size_t respSize);
static void cmd_stop_instance(PipeServer* server, const char* payload, char* response,
                              size_t respSize);
static void cmd_restart_instance(PipeServer* server, const char* payload, char* response,
                                 size_t respSize);
static void cmd_reload_config(PipeServer* server, char* response, size_t respSize);
static void cmd_get_logs(PipeServer* server, const char* payload, char* response, size_t respSize);
 ...
 */
static BOOL
build_pipe_security_attributes(SECURITY_ATTRIBUTES* sa, SECURITY_DESCRIPTOR* sd, PACL* aclOut)
{
	/*
	 ...
	 */
	DWORD aclSize = (DWORD)(sizeof(ACL) + 3 * (sizeof(ACCESS_ALLOWED_ACE) - sizeof(DWORD)) +
	...
	{
		PSID sid = NULL;
		if (!AllocateAndInitializeSid(&ntAuth, 1, SECURITY_LOCAL_SYSTEM_RID, 0, 0, 0, 0, 0, 0, 0,
		                              &sid))
			goto fail;
		AddAccessAllowedAceEx(acl, ACL_REVISION, 0, GENERIC_READ | GENERIC_WRITE, sid);
		FreeSid(sid);
	}
	...
	{
		PSID sid = NULL;
		if (!AllocateAndInitializeSid(&ntAuth, 1, SECURITY_NETWORK_SERVICE_RID, 0, 0, 0, 0, 0, 0, 0,
		                              &sid))
			goto fail;
		AddAccessAllowedAceEx(acl, ACL_REVISION, 0, GENERIC_READ | GENERIC_WRITE, sid);
		FreeSid(sid);
	...
	/* ── Current interactive user (console session) ───────────── */
	{
		...
		else
		{
			...
			PSID sid = NULL;
			if (AllocateAndInitializeSid(&ntAuth, 1, SECURITY_INTERACTIVE_RID, 0, 0, 0, 0, 0, 0, 0,
			                             &sid))
			{
				AddAccessAllowedAceEx(acl, ACL_REVISION, 0, GENERIC_READ | GENERIC_WRITE, sid);
				FreeSid(sid);
 ...
 */
static HANDLE
create_pipe_instance(const char* pipeName)
{
 ...
 */
static void
client_remove(PipeServer* server, HANDLE hPipe)
{
 ...
 */
static size_t
json_escape_string(const char* input, char* output, size_t outSize)
{
 ...
 */
static int
extract_command(const char* payload)
{
	if (!payload)
		return -1;
 ...
 */
static int
extract_instance_name(const char* payload, char* name, size_t nameSize)
{
 ...
 */
static DWORD WINAPI
client_handler(LPVOID arg)
{
	ClientConnection* client = (ClientConnection*)arg;
	PipeServer* server = client->server;
	HANDLE hPipe = client->hPipe;

	LOG_I("pipe_server", "Client handler thread started (pipe=%p, threadId=%lu)", (void*)hPipe,
	      GetCurrentThreadId());
	...
	while (server->running)
	{
		...
		if (!recvResult)
		{
			...
			{
				LOG_D("pipe_server", "pipe_frame_recv error %lu on pipe %p", err, (void*)hPipe);
			}
			break;
		}
		...
		LOG_I("pipe_server", "Client handler: received command %d, payload='%s'", cmd, payload);
		...
		if (responseLen > 0)
		{
			LOG_D("pipe_server", "Client handler: sending response (%lu bytes): %s", responseLen,
			      response);
			...
			if (!sendResult)
			{
				LOG_D("pipe_server", "pipe_frame_send error %lu on pipe %p", GetLastError(),
				      (void*)hPipe);
				break;
			}
		}
	...
	LOG_I("pipe_server", "Client handler: exiting loop (recvResult=%d, lastError=%lu)", recvResult,
	      GetLastError());
...
listener_thread(LPVOID arg)
{
	...
	while (server->running)
	{
		...
		ClientConnection* client = (ClientConnection*)calloc(1, sizeof(ClientConnection));
		if (!client)
		{
		...
		if (!hThread)
		{
			LOG_E("pipe_server", "CreateThread for client failed: %lu", GetLastError());
			CloseHandle(hPipe);
			free(client);
			continue;
		}
 ...
 */
static void
cmd_list_instances(PipeServer* server, char* response, size_t respSize)
{
	if (!server->mgr)
	{
	...
	for (unsigned int i = 0; i < server->mgr->instanceCount; i++)
	{
		...
		int written = _snprintf(array + pos, sizeof(array) - pos,
 ...
 */
static void
cmd_start_instance(PipeServer* server, const char* payload, char* response, size_t respSize)
{
	if (!server->mgr)
	{
		_snprintf(response, respSize,
 ...
 */
static void
cmd_stop_instance(PipeServer* server, const char* payload, char* response, size_t respSize)
{
	if (!server->mgr)
	{
		_snprintf(response, respSize,
 ...
 */
static void
cmd_restart_instance(PipeServer* server, const char* payload, char* response, size_t respSize)
{
	if (!server->mgr)
	{
		_snprintf(response, respSize,
 ...
 */
static void
cmd_reload_config(PipeServer* server, char* response, size_t respSize)
{
	if (!server->mgr)
	{
		_snprintf(response, respSize,
	...
	const char* configPath =
	    server->mgr->configPath ? server->mgr->configPath : "C:\\ProgramData\\OmniRDP\\config.ini";
 ...
 */
static void
cmd_get_logs(PipeServer* server, const char* payload, char* response, size_t respSize)
{
	...
	{
		long chunkSize = (readPos >= (long)TAIL_CHUNK_SIZE) ? (long)TAIL_CHUNK_SIZE : readPos;
		readPos -= chunkSize;
 ...
 */
static BOOL
push_write_frame(HANDLE hPipe, const char* json, DWORD jsonLen)
{
	DWORD lenLe = jsonLen;
	DWORD written = 0;
 ...
 */
static void
push_to_all(PipeServer* server, const char* json, DWORD jsonLen)
{
	...
	while (curr)
	{
		...
		if (!push_write_frame(curr->hPipe, json, jsonLen))
		{
			...
			LOG_D("pipe_server", "Removing dead client (%p) during push", (void*)curr->hPipe);
...
int
pipe_server_init(PipeServer* server, const char* pipeName, InstanceManager* mgr)
{
	...
	_snprintf(server->pipeName, sizeof(server->pipeName), "\\\\.\\pipe\\%s", pipeName);
	...
	server->hListenerThread = CreateThread(NULL, 0, listener_thread, server, 0, NULL);
...
void
pipe_server_stop(PipeServer* server)
{
	if (!server)
		return;
...
void
pipe_server_push_stats(PipeServer* server)
{
	...
	for (unsigned int i = 0; i < server->mgr->instanceCount; i++)
	{
		...
		n = _snprintf(json + pos, sizeof(json) - pos,
		              "%s\"%s\":{\"state\":%d,\"viewer_count\":%lu}", first ? "" : ",", info.name,
		              (int)info.state, info.viewer_count);
...
void
pipe_server_push_event(PipeServer* server, const char* eventType, const char* instanceName)
{
