...
static void
backend_on_channel_disconnected(void *context,
...
static BOOL on_primary_order_info(rdpContext *context,
                                  const ORDER_INFO *order_info,
                                  const char *order_name);
static BOOL on_secondary_cache_order_info(rdpContext *context,
                                          INT16 orderLength, UINT16 extraFlags,
                                          UINT8 orderType,
...
static BOOL backend_configure_addin_dll_directory(void) {
  ...
  if (!GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
      ...
      !module) {
    WLog_WARN(TAG,
              "Unable to resolve FreeRDP runtime module for addin loading");
    return FALSE;
  }
  ...
  if ((length == 0) || (length >= ARRAYSIZE(path))) {
    WLog_WARN(TAG,
              "Unable to resolve FreeRDP runtime directory for addin loading");
    return FALSE;
  }
  ...
  if (!separator) {
    WLog_WARN(TAG,
              "Unable to parse FreeRDP runtime directory for addin loading");
    return FALSE;
  }
  ...
  *separator = '\0';
  if (!SetDllDirectoryA(path)) {
    WLog_WARN(TAG,
              "Failed to add FreeRDP runtime directory to DLL search path: %s",
              path);
    return FALSE;
  }
...
static BOOL backend_prepare_rdpgfx_channels(BackendClient *client) {
  ...
  if (!freerdp_settings_set_bool(settings, FreeRDP_NetworkAutoDetect, FALSE) ||
      !freerdp_settings_set_bool(settings, FreeRDP_SupportHeartbeatPdu,
                                 FALSE) ||
      !freerdp_settings_set_bool(settings, FreeRDP_SupportMultitransport,
                                 FALSE) ||
  ...
  if (freerdp_settings_get_bool(settings, FreeRDP_SupportGraphicsPipeline) &&
      !freerdp_client_add_dynamic_channel(
          settings, sizeof(rdpgfx) / sizeof(rdpgfx[0]), rdpgfx)) {
...
static void backend_attach_rdpgfx_context(BackendClient *client,
  ...
  client->rdpgfx->DeleteEncodingContext =
...
static BOOL backend_service_full_refresh_request(BackendClient *client) {
  ...
  should_request =
      client->full_refresh_requested && !client->full_refresh_in_flight;
  ...
  if ((width == 0) || (height == 0) || (width > UINT16_MAX) ||
    ...
    if (client->full_refresh_requested && !client->full_refresh_in_flight &&
      ...
      backend_complete_full_refresh_locked(
          client, generation, BACKEND_FULL_REFRESH_OUTCOME_FAILED, now);
    }
    LeaveCriticalSection(&client->refresh_lock);
    WLog_WARN(TAG,
  ...
  if (!sent) {
    WLog_WARN(TAG,
              "Backend full refresh generation=%" PRIu64 " failed for %ux%u",
              generation, width, height);
    return FALSE;
  }
...
void backend_store_pointer_position(BackendClient *client, UINT16 x, UINT16 y) {
  ...
  WLog_INFO(TAG, "backend_store_pointer_position: x=%u y=%u gen=%llu->%llu", x,
            y, (unsigned long long)gen_before, (unsigned long long)gen_after);
}
...
static BOOL backend_activate_pointer_shape(BackendClient *client,
                                           PointerShapeEntry *entry) {
  if (!client || !entry)
    return FALSE;
...
static void backend_store_desktop_layout(BackendClient *client, UINT32 width,
                                         UINT32 height, BOOL *changed_out) {
  BOOL changed = FALSE;
...
static void backend_refresh_desktop_layout(BackendClient *client,
                                           rdpContext *context) {
  UINT32 width = 0;
  UINT32 height = 0;
  BOOL changed = FALSE;
  ...
  if (context && context->gdi && context->gdi->width > 0 &&
      context->gdi->height > 0) {
  ...
  settings = context ? context->settings
                     : (client->context ? client->context->settings : NULL);
  ...
  if (changed) {
    ...
    WLog_INFO(TAG, "Desktop layout updated to %ux%u generation=%" PRIu32, width,
              height, generation);
    viewer_server_notify_backend_layout_change(client, width, height,
                                               generation);
  }
}

static BOOL backend_forward_surface_bits(BackendClient *client,
...
static BOOL backend_forward_frame_marker(BackendClient *client,
...
backend_rdpgfx_caps_confirm(RdpgfxClientContext *context,
  ...
  WLog_INFO(TAG,
            "Backend RDPEGFX caps confirmed version=0x%08" PRIX32
            " flags=0x%08" PRIX32,
...
static BOOL
on_pointer_position(rdpContext *context,
...
static BOOL on_pointer_system(rdpContext *context,
  ...
  WLog_INFO(TAG, "PointerSystem type=0x%08" PRIX32 " visible=%d",
            pointer_system->type, visible);
  return TRUE;
}

static BOOL on_pointer_color(rdpContext *context,
  ...
  EnterCriticalSection(&client->pointer_lock);
  entry = pointer_shape_cache_add_from_color(client->pointer_shape_cache,
                                             pointer_color);
  LeaveCriticalSection(&client->pointer_lock);
  ...
  if (!entry) {
    WLog_WARN(
        TAG,
        "PointerColor cache=%" PRIu16 " size=%" PRIu16 "x%" PRIu16 " rejected",
  ...
  WLog_INFO(TAG, "PointerColor cache=%" PRIu16 " size=%" PRIu16 "x%" PRIu16,
            pointer_color->cacheIndex, pointer_color->width,
            pointer_color->height);
  return TRUE;
}

static BOOL on_pointer_new(rdpContext *context,
                           const POINTER_NEW_UPDATE *pointer_new) {
  const POINTER_COLOR_UPDATE *color =
      pointer_new ? &pointer_new->colorPtrAttr : NULL;
  BackendClient *client = g_backend_client;
  PointerShapeEntry *entry = NULL;
  ...
  EnterCriticalSection(&client->pointer_lock);
  entry = pointer_shape_cache_add_from_new(client->pointer_shape_cache,
                                           pointer_new);
  LeaveCriticalSection(&client->pointer_lock);

  if (!entry) {
    WLog_WARN(TAG,
              "PointerNew cache=%" PRIu16 " size=%" PRIu16 "x%" PRIu16
              " xorBpp=%" PRIu32 " rejected",
              color->cacheIndex, color->width, color->height,
              pointer_new->xorBpp);
    return FALSE;
  }
...
static BOOL on_pointer_cached(rdpContext *context,
  ...
  if (!entry) {
    WLog_WARN(TAG, "PointerCached cache=%" PRIu32 " missing",
...
static BOOL on_bitmap_update(rdpContext *context, const BITMAP_UPDATE *bitmap) {
  ...
  if (rc && bitmap) {
    const BITMAP_DATA *first =
        (bitmap->number > 0) ? &bitmap->rectangles[0] : NULL;
    ...
    if (backend_should_log_path_event(client->bitmap_update_count) ||
        backend_should_log_bitmap_perf(client->bitmap_update_batches_total,
                                       callback_us, publish_us)) {
      WLog_INFO(TAG,
                "Received BitmapUpdate count=%" PRIu64 " rectangles=%" PRIu32
                " skipCompression=%d firstRect=(%" PRIu32 ",%" PRIu32
                ")-(%" PRIu32 ",%" PRIu32 ") size=%" PRIu32 "x%" PRIu32
                " bpp=%" PRIu32 " compressed=%d payload=%" PRIu32
                " batch=%" PRIu64 " batchBytes=%" PRIu64 " callbackUs=%" PRIu64
                " publishUs=%" PRIu64 " publishOk=%d",
                client->bitmap_update_count, bitmap->number,
                bitmap->skipCompression, first ? first->destLeft : 0,
                first ? first->destTop : 0, first ? first->destRight : 0,
                first ? first->destBottom : 0, first ? first->width : 0,
                first ? first->height : 0, first ? first->bitsPerPixel : 0,
                first ? first->compressed : 0, first ? first->bitmapLength : 0,
                client->bitmap_update_batches_total, total_bytes, callback_us,
                publish_us, forwarded);
    }
...
static BOOL on_primary_order_info(rdpContext *context,
  ...
  if (rc) {
    ...
    if (backend_should_log_path_event(client->primary_order_info_count)) {
      WLog_INFO(TAG,
                "Received primary drawing order count=%" PRIu64
                " order=%s type=%" PRIu32,
                client->primary_order_info_count,
...
static BOOL on_secondary_cache_order_info(rdpContext *context,
  ...
  if (rc) {
    ...
    if (backend_should_log_path_event(
            client->secondary_cache_order_info_count)) {
      WLog_INFO(TAG,
                "Received secondary/cache order count=%" PRIu64
                " order=%s type=%" PRIu8 " length=%" PRIi16
                " extraFlags=%" PRIu16,
                client->secondary_cache_order_info_count,
                orderName ? orderName : "unknown", orderType, orderLength,
                extraFlags);
    }
  }
...
static BOOL on_surface_frame_marker(rdpContext *context,
                                    const SURFACE_FRAME_MARKER *marker) {
  (void)context;
  WLog_INFO(TAG, "FrameMarker action=%u id=%u", marker->frameAction,
            marker->frameId);
  return TRUE;
}
...
static BOOL
on_surface_frame_marker_wrapped(rdpContext *context,
                                const SURFACE_FRAME_MARKER *marker) {
  BackendClient *client = g_backend_client;
  BOOL rc = FALSE;
...
static BOOL on_post_connect(freerdp *instance) {
  ...
  if (client->context && client->context->update) {
    ...
    client->orig_surface_frame_marker =
        client->context->update->SurfaceFrameMarker;
    client->context->update->SurfaceFrameMarker =
        on_surface_frame_marker_wrapped;
    if (client->context->update->primary) {
      client->orig_primary_order_info =
          client->context->update->primary->OrderInfo;
...

  {
    ...
    if (s) {
      ...
      UINT32 negoFlags =
          freerdp_settings_get_uint32(s, FreeRDP_NegotiationFlags);
      UINT32 k = 0;
      WLog_INFO(
          TAG,
          "on_post_connect: Negotiated settings: DesktopWidth=%" PRIu32
          ", DesktopHeight=%" PRIu32
          ", UseMultimon=%s, ForceMultimon=%s, SpanMonitors=%s, "
          ...
          freerdp_settings_get_bool(s, FreeRDP_ForceMultimon) ? "TRUE"
                                                              : "FALSE",
          freerdp_settings_get_bool(s, FreeRDP_SpanMonitors) ? "TRUE" : "FALSE",
          freerdp_settings_get_bool(s, FreeRDP_SupportMonitorLayoutPdu)
              ? "TRUE"
              : "FALSE",
          monCount,
      ...
      for (k = 0; k < monCount; k++) {
        ...
        if (m)
          WLog_INFO(TAG,
                    "on_post_connect:   monitor[%" PRIu32 "]: x=%" PRId32
...
static BOOL on_authenticate_ex(freerdp *instance, char **username,
...
static DWORD on_verify_certificate_ex(freerdp *instance, const char *host,
...
void backend_set_monitor_count(BackendClient *client, UINT32 monitor_count) {
  ...
  WLog_INFO(TAG, "backend_set_monitor_count: monitor_count=%" PRIu32,
            monitor_count);
  ...
  WLog_INFO(TAG,
            "backend_set_monitor_count: layout: total_width=%" PRIu32
            ", total_height=%" PRIu32 ", monitor_count=%" PRIu32,
  ...
  if (client->context && client->context->settings) {
    ...
    if (client->monitor_layout.monitor_count > 1) {
      ...
      WLog_INFO(TAG,
                ...
                "DesktopWidth=%" PRIu32 ", DesktopHeight=%" PRIu32
                " for %" PRIu32 " monitors",
                client->monitor_layout.total_width,
                client->monitor_layout.total_height, count);
...

      {
        ...
        for (k = 0; k < monCountAfter; k++) {
          ...
          if (m)
            WLog_INFO(TAG,
                      "backend_set_monitor_count:   settings[%" PRIu32
      ...
      WLog_INFO(TAG,
                ...
                "ForceMultimon=FALSE, DesktopWidth=%" PRIu32
                ", DesktopHeight=%" PRIu32,
                client->monitor_layout.total_width,
                client->monitor_layout.total_height);
    }
...
BackendClient *backend_init(void) {
  ...
  if (!client->context->pubSub ||
      PubSub_SubscribeChannelConnected(client->context->pubSub,
                                       backend_on_channel_connected) < 0 ||
      PubSub_SubscribeChannelDisconnected(
          client->context->pubSub, backend_on_channel_disconnected) < 0) {
    freerdp_context_free(instance);
    freerdp_free(instance);
    pointer_shape_cache_free(client->pointer_shape_cache);
    DeleteCriticalSection(&client->layout_lock);
  ...
  WLog_INFO(TAG,
            "backend_init: Initial monitor layout: total_width=%" PRIu32
            ", total_height=%" PRIu32 ", monitor_count=%" PRIu32,
            client->monitor_layout.total_width,
            client->monitor_layout.total_height,
            client->monitor_layout.monitor_count);
  WLog_INFO(TAG,
            "backend_init: Setting DesktopWidth=%" PRIu32
            ", DesktopHeight=%" PRIu32 ", ColorDepth=32",
            client->monitor_layout.total_width,
            client->monitor_layout.total_height);
  ...
  if (client->monitor_layout.monitor_count > 1) {
    ...
    WLog_INFO(TAG,
              ...
              "SupportMonitorLayoutPdu=TRUE, ForceMultimon=TRUE for %" PRIu32
              " monitors",
              count);
    ...
    for (i = 0; i < count; i++) {
      ...
      WLog_INFO(TAG,
                ...
                i, monitors[i].x, monitors[i].y, monitors[i].width,
                monitors[i].height, monitors[i].is_primary ? "TRUE" : "FALSE");
    }
    freerdp_settings_set_monitor_def_array_sorted(settings, monitors, count);

    {
      ...
      for (k = 0; k < monCountAfter; k++) {
        ...
        if (m)
          WLog_INFO(TAG,
                    "backend_init:   settings[%" PRIu32 "]: x=%" PRId32
                    ", y=%" PRId32 ", width=%" PRId32 ", height=%" PRId32
                    ", is_primary=%s",
                    k, m->x, m->y, m->width, m->height,
                    m->is_primary ? "TRUE" : "FALSE");
...
BOOL backend_configure(BackendClient *client, const char *hostname, UINT16 port,
  ...
  /* TODO: hook certificate validation policy here; keep auth policy config for
   * later.
   */
...
BOOL backend_abandon_full_refresh_if_timed_out(BackendClient *client,
  ...
  if ((started_ts > 0) && ((now - started_ts) >= timeout_ms)) {
    backend_complete_full_refresh_locked(
        client, generation, BACKEND_FULL_REFRESH_OUTCOME_TIMED_OUT, now);
    timed_out = TRUE;
  }
  LeaveCriticalSection(&client->refresh_lock);
...
void backend_get_pointer_state(BackendClient *client, UINT16 *x, UINT16 *y,
  ...
  backend_get_pointer_snapshot(client, x, y, visible, type, NULL, &position_gen,
                               &shape_gen);
  if (generation)
