...
static LRESULT CALLBACK status_dlg_proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
 ...
 */
static const char*
state_to_string(PipeInstanceState state)
{
	switch (state)
	{
 ...
 */
static void
populate_listview(HWND hListView, const TrayAppCtx* ctx)
{
	int si, ii;
	...
	for (si = 0; si < ctx->serviceCount; si++)
	{
		...
		for (ii = 0; ii < svc->instanceCount; ii++)
		{
			...
			lParam = (LPARAM)((si << LPARAM_SERVICE_SHIFT) | (ii & LPARAM_INSTANCE_MASK));
			...
			ListView_SetItemText(hListView, itemIndex, 2, (LPSTR)state_to_string(inst->state));
 ...
 */
static void
update_button_states(HWND hwnd, TrayAppCtx* ctx)
{
	HWND hListView;
	int selIndex;
	BOOL hasSelection;
	...
	if (hasSelection && ctx)
	{
		...
		if (ListView_GetItem(hListView, &lvi))
		{
			    ...
			    ii < ctx->services[si].instanceCount)
			{
				...
				canRestart = (state == INSTANCE_RUNNING || state == INSTANCE_RECONNECTING);
			}
		}
	}
 ...
 */
static void
send_instance_command(HWND hwnd, TrayAppCtx* ctx, PipeCommand cmd)
{
	HWND hListView;
	int selIndex;
	LVITEMA lvi;
	...
	if (si < 0 || si >= ctx->serviceCount || ii < 0 || ii >= ctx->services[si].instanceCount)
		return;

	/* Build the request */
	memset(&req, 0, sizeof(req));
	req.command = cmd;
	strncpy(req.instance_name, ctx->services[si].instances[ii].name, sizeof(req.instance_name) - 1);
	...
	LOG_I(LOG_TAG, "Sending '%s' command for instance '%s'", cmdName, req.instance_name);

	if (pipe_client_send_request(&ctx->services[si].client, &req, &resp, 0) != 0)
	{
		LOG_E(LOG_TAG, "Failed to send '%s' command for '%s'", cmdName, req.instance_name);
		return;
	}

	if (!resp.success)
	{
		LOG_W(LOG_TAG, "'%s' command failed for '%s': %s", cmdName, req.instance_name,
		      resp.error_message);
	}
	else
	{
		LOG_I(LOG_TAG, "'%s' command succeeded for '%s'", cmdName, req.instance_name);
	}
}
...
status_dlg_proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
		/* ── WM_CREATE ──────────────────────────────────────────── */
		case WM_CREATE:
		{
			...
			hListView = CreateWindowExA(
			    0, WC_LISTVIEW, "", WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_SINGLESEL, 0, 0, 100,
			    100, hwnd, (HMENU)(INT_PTR)IDC_LISTVIEW, cs->hInstance, NULL);
			...
			/* ── Buttons ─────────────────────────────────────────── */
			CreateWindowExA(0, "BUTTON", "Start", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0, 0, 70,
			                25, hwnd, (HMENU)(INT_PTR)IDC_START, cs->hInstance, NULL);

			CreateWindowExA(0, "BUTTON", "Stop", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0, 0, 70,
			                25, hwnd, (HMENU)(INT_PTR)IDC_STOP, cs->hInstance, NULL);

			CreateWindowExA(0, "BUTTON", "Restart", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0, 0, 70,
			                25, hwnd, (HMENU)(INT_PTR)IDC_RESTART, cs->hInstance, NULL);

			CreateWindowExA(0, "BUTTON", "Refresh", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0, 0, 70,
			                25, hwnd, (HMENU)(INT_PTR)IDC_REFRESH, cs->hInstance, NULL);

			CreateWindowExA(0, "BUTTON", "Close", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0, 0, 70,
			                25, hwnd, (HMENU)(INT_PTR)IDC_CLOSE, cs->hInstance, NULL);
		...
		case WM_SIZE:
		{
			...
			SetWindowPos(GetDlgItem(hwnd, IDC_START), NULL, startX, btnY, btnWidth, btnHeight,
			             SWP_NOZORDER);
			SetWindowPos(GetDlgItem(hwnd, IDC_STOP), NULL, startX + 1 * (btnWidth + gap), btnY,
			             btnWidth, btnHeight, SWP_NOZORDER);
			SetWindowPos(GetDlgItem(hwnd, IDC_RESTART), NULL, startX + 2 * (btnWidth + gap), btnY,
			             btnWidth, btnHeight, SWP_NOZORDER);
			SetWindowPos(GetDlgItem(hwnd, IDC_REFRESH), NULL, startX + 3 * (btnWidth + gap), btnY,
			             btnWidth, btnHeight, SWP_NOZORDER);
			SetWindowPos(GetDlgItem(hwnd, IDC_CLOSE), NULL, startX + 4 * (btnWidth + gap), btnY,
			             btnWidth, btnHeight, SWP_NOZORDER);
...
HWND
tray_status_dlg_show(TrayAppCtx* ctx, HINSTANCE hInstance)
{
	HWND hwnd;
	...
	hwnd = CreateWindowExA(0, "OmniRDPStatusDlg", "OmniRDP Status", WS_OVERLAPPEDWINDOW,
...
void
tray_status_dlg_refresh(TrayAppCtx* ctx)
{
	HWND hListView;
...
BOOL
tray_status_dlg_is_visible(void)
{
	return (g_hDlg && IsWindowVisible(g_hDlg)) ? TRUE : FALSE;
}

HWND
tray_status_dlg_get_hwnd(void)
{
	return g_hDlg;
}
