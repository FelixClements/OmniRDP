...
populate_listview(HWND hListView, const TrayAppCtx *ctx)
{
    ...
    for (si = 0; si < ctx->serviceCount; si++)
    {
        ...
        for (ii = 0; ii < svc->instanceCount; ii++)
        {
            ...
            lParam = (LPARAM)((si << LPARAM_SERVICE_SHIFT) |
                              (ii & LPARAM_INSTANCE_MASK));
            ...
            {
                _snprintf(viewers, sizeof(viewers), "%lu/10",
                          inst->viewer_count);
                viewers[sizeof(viewers) - 1] = '\0';
            }

            /* Backend column */
            _snprintf(backend, sizeof(backend), "%s:%u",
...
update_button_states(HWND hwnd, TrayAppCtx *ctx)
{
    ...
    if (hasSelection && ctx)
    {
        ...
        if (ListView_GetItem(hListView, &lvi))
        {
            ...
            if (si >= 0 && si < ctx->serviceCount &&
                ii >= 0 && ii < ctx->services[si].instanceCount)
            {
                const PipeInstanceState state =
                    ctx->services[si].instances[ii].state;

                canStart   = (state == INSTANCE_STOPPED);
                canStop    = (state == INSTANCE_RUNNING ||
                              state == INSTANCE_RECONNECTING);
                canRestart = (state == INSTANCE_RUNNING ||
                              state == INSTANCE_RECONNECTING);
            }
        }
    }
...
send_instance_command(HWND hwnd, TrayAppCtx *ctx, PipeCommand cmd)
{
    ...
    switch (cmd)
    {
        case PIPE_CMD_START_INSTANCE:   cmdName = "start";   break;
        case PIPE_CMD_STOP_INSTANCE:    cmdName = "stop";    break;
        case PIPE_CMD_RESTART_INSTANCE: cmdName = "restart"; break;
        default:                        cmdName = "unknown"; break;
    }

    LOG_I(LOG_TAG, "Sending '%s' command for instance '%s'",
          cmdName, req.instance_name);

    if (pipe_client_send_request(&ctx->services[si].client, &req, &resp, 0) != 0)
    {
        LOG_E(LOG_TAG, "Failed to send '%s' command for '%s'",
              cmdName, req.instance_name);
        return;
    }
    ...
    if (!resp.success)
    {
        LOG_W(LOG_TAG, "'%s' command failed for '%s': %s",
              cmdName, req.instance_name, resp.error_message);
    }
    else
    {
        LOG_I(LOG_TAG, "'%s' command succeeded for '%s'",
              cmdName, req.instance_name);
    }
}
    ...
    switch (msg)
    {
        ...
        case WM_CREATE:
        {
            ...
            CreateWindowExA(
                ...
                0, 0, 70, 25, hwnd,
                (HMENU)(INT_PTR)IDC_START, cs->hInstance, NULL);

            CreateWindowExA(
                0, "BUTTON", "Stop",
                WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                0, 0, 70, 25, hwnd,
                (HMENU)(INT_PTR)IDC_STOP, cs->hInstance, NULL);
            ...
            CreateWindowExA(
                ...
                WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
                0, 0, 70, 25, hwnd,
                (HMENU)(INT_PTR)IDC_CLOSE, cs->hInstance, NULL);
        ...
        case WM_SIZE:
        {
            ...
            SetWindowPos(GetDlgItem(hwnd, IDC_START), NULL,
                         startX, btnY, btnWidth, btnHeight, SWP_NOZORDER);
            SetWindowPos(GetDlgItem(hwnd, IDC_STOP), NULL,
                         startX + 1 * (btnWidth + gap),
                         btnY, btnWidth, btnHeight, SWP_NOZORDER);
            SetWindowPos(GetDlgItem(hwnd, IDC_RESTART), NULL,
                         startX + 2 * (btnWidth + gap),
                         btnY, btnWidth, btnHeight, SWP_NOZORDER);
            SetWindowPos(GetDlgItem(hwnd, IDC_REFRESH), NULL,
                         startX + 3 * (btnWidth + gap),
                         btnY, btnWidth, btnHeight, SWP_NOZORDER);
            SetWindowPos(GetDlgItem(hwnd, IDC_CLOSE), NULL,
                         startX + 4 * (btnWidth + gap),
                         btnY, btnWidth, btnHeight, SWP_NOZORDER);
        ...
        case WM_NOTIFY:
        {
            ...
            {
                TrayAppCtx *ctx = (TrayAppCtx *)GetWindowLongPtr(
                                      hwnd, GWLP_USERDATA);
        ...
        case WM_COMMAND:
        {
            ...
            {
                case IDC_START:
                    send_instance_command(hwnd, ctx,
...
tray_status_dlg_show(TrayAppCtx *ctx, HINSTANCE hInstance)
{
    ...
    hwnd = CreateWindowExA(
        ...
        NULL,        /* no menu */
        hInstance,
        (LPVOID)ctx  /* passed to WM_CREATE via CREATESTRUCT */
    );

    if (!hwnd)
    {
        LOG_E(LOG_TAG, "Failed to create status dialog: %lu",
              GetLastError());
        return NULL;
    }
