...
static const char*
instance_state_str(PipeInstanceState state)
{
    switch (state)
    {
        case INSTANCE_STOPPED:      return "Stopped";
        case INSTANCE_STARTING:     return "Starting";
        case INSTANCE_RUNNING:      return "Running";
        case INSTANCE_RECONNECTING: return "Reconnecting";
...
add_tray_icon(TrayAppCtx *ctx)
{
    ...
    {
        LOG_W(LOG_TAG, "Shell_NotifyIconA(NIM_ADD) failed: %lu",
              GetLastError());
    }
}

static void
remove_tray_icon(TrayAppCtx *ctx)
{
    NOTIFYICONDATAA nid;
    memset(&nid, 0, sizeof(nid));
    nid.cbSize = sizeof(NOTIFYICONDATAA);
 ...
 */
static DWORD WINAPI
push_listener_thread(LPVOID param)
{
    ...
    while (ctx->running) {
        ...
        for (unsigned int i = 0; i < ctx->serviceCount; i++) {
            if (!ctx->services[i].connected) continue;

            PipePushType pushType;
            char *payload = NULL;
            DWORD payloadLen = 0;

            if (pipe_client_recv_push(&ctx->services[i].client, &pushType,
...
int
tray_icon_init(TrayAppCtx *ctx, HINSTANCE hInstance)
{
    if (!ctx || !hInstance)
        return -1;
    ...
    ctx->hwnd = CreateWindowExA(
        0, HIDDEN_WIN_CLASS, "OmniRDP Tray",
        WS_OVERLAPPEDWINDOW,
    ...
    ctx->hIcons[1] = create_color_icon(RGB(255, 200, 0));   /* Yellow — starting/reconnecting */
    ctx->hIcons[2] = create_color_icon(RGB(220, 0, 0));     /* Red — stopped/error */
    ctx->hIcons[3] = create_color_icon(RGB(128, 128, 128)); /* Gray — no services */
    ...
    if (!ctx->hPollThread)
    {
        LOG_E(LOG_TAG, "CreateThread(poll_thread) failed: %lu",
              GetLastError());
...
void
tray_icon_update(TrayAppCtx *ctx)
{
    if (!ctx)
        return;
    ...
    _snprintf(nid.szTip, sizeof(nid.szTip),
              "OmniRDP: %u/%u running", runningCount, totalCount);
    nid.szTip[sizeof(nid.szTip) - 1] = '\0';

    if (!Shell_NotifyIconA(NIM_MODIFY, &nid))
    {
        LOG_W(LOG_TAG, "Shell_NotifyIconA(NIM_MODIFY) failed: %lu",
              GetLastError());
    }
...
void
tray_icon_show_menu(TrayAppCtx *ctx, HWND hwnd)
{
    if (!ctx || !hwnd)
        return;
    ...
    /* ── Header (disabled) ────────────────────────────────────── */
    AppendMenuA(hMenu, MF_STRING | MF_GRAYED | MF_DISABLED,
                0, "OmniRDP Service");
    ...
    if (ctx->serviceCount > 0 && hServicesMenu)
    {
        for (unsigned int i = 0; i < ctx->serviceCount; i++)
        {
            ...
            char svcHeader[280];
            _snprintf(svcHeader, sizeof(svcHeader), "%s",
                      svc->serviceName);
            svcHeader[sizeof(svcHeader) - 1] = '\0';
            AppendMenuA(hSvcMenu, MF_STRING | MF_GRAYED | MF_DISABLED,
                        0, svcHeader);
            ...
            for (unsigned int j = 0; j < svc->instanceCount; j++)
            {
                ...
                char instLabel[192];
                _snprintf(instLabel, sizeof(instLabel),
                          "%s [%s]", inst->name, stateStr);
                instLabel[sizeof(instLabel) - 1] = '\0';
                AppendMenuA(hInstMenu,
                            MF_STRING | MF_GRAYED | MF_DISABLED,
                            0, instLabel);

                /* Action items */
                UINT baseId = IDM_SERVICE_BASE
                            + (i * IDM_SERVICE_STRIDE)
                            + (j * IDM_INSTANCE_SLOT_SIZE);

                if (inst->state == INSTANCE_RUNNING)
                {
                    AppendMenuA(hInstMenu, MF_STRING,
                                baseId + IDM_INSTANCE_ACTION_STOP,
                                "Stop");
                    AppendMenuA(hInstMenu, MF_STRING,
                                baseId + IDM_INSTANCE_ACTION_RESTART,
                                "Restart");
                }
                else if (inst->state == INSTANCE_STOPPED)
                {
                    AppendMenuA(hInstMenu, MF_STRING,
                                baseId + IDM_INSTANCE_ACTION_START,
                                "Start");
                }
                ...
                _snprintf(instMenuText, sizeof(instMenuText),
                          "%s [%s]", inst->name, stateStr);
                instMenuText[sizeof(instMenuText) - 1] = '\0';

                AppendMenuA(hSvcMenu, MF_POPUP | MF_STRING,
                            (UINT_PTR)hInstMenu, instMenuText);
            }

            /* Add the service submenu to the services menu */
            AppendMenuA(hServicesMenu, MF_POPUP | MF_STRING,
                        (UINT_PTR)hSvcMenu, svc->serviceName);
        }
    }
    else
    {
        /* No services found */
        AppendMenuA(hServicesMenu, MF_STRING | MF_GRAYED | MF_DISABLED,
                    0, "(No services found)");
    }

    LeaveCriticalSection(&ctx->lock);

    AppendMenuA(hMenu, MF_POPUP | MF_STRING,
                (UINT_PTR)hServicesMenu, "Services");

    /* ── Separator ────────────────────────────────────────────── */
    AppendMenuA(hMenu, MF_SEPARATOR, 0, NULL);

    /* ── Global Actions ───────────────────────────────────────── */
    AppendMenuA(hMenu, MF_STRING, IDM_RELOAD_CONFIG,
                "Reload All Configurations");
    AppendMenuA(hMenu, MF_STRING, IDM_VIEW_LOG,
                "View Service Log");

    /* "Install Service..." shown only when none discovered */
    if (ctx->serviceCount == 0)
    {
        AppendMenuA(hMenu, MF_STRING, IDM_INSTALL_SERVICE,
                    "Install Service...");
    }
...
void
tray_icon_discover_services(TrayAppCtx *ctx)
{
    if (!ctx)
        return;
    ...
    LPENUM_SERVICE_STATUS_PROCESSA services =
        (LPENUM_SERVICE_STATUS_PROCESSA)HeapAlloc(
    ...
    if (!EnumServicesStatusExA(
            ...
            NULL))
    {
        LOG_W(LOG_TAG, "EnumServicesStatusExA (data) failed: %lu",
              GetLastError());
    ...
    for (DWORD i = 0;
         i < servicesReturned && ctx->serviceCount < 8;
         i++)
    {
...
void
tray_icon_refresh_instances(TrayAppCtx *ctx)
{
    if (!ctx)
        return;
    ...
    for (unsigned int i = 0; i < ctx->serviceCount; i++)
    {
        ...
        if (!resp.success)
        {
            LOG_W(LOG_TAG, "LIST_INSTANCES failed for '%s': %s",
        ...
        LOG_D(LOG_TAG, "Refreshed %u instances for '%s'",
              svc->instanceCount, svc->serviceName);
    }
...
void
tray_icon_cleanup(TrayAppCtx *ctx)
{
    if (!ctx)
        return;
...
static void
tray_on_command(TrayAppCtx *ctx, WPARAM wParam)
{
    UINT id = LOWORD(wParam);
    ...
    switch (id)
    {
        ...
        case IDM_RELOAD_CONFIG:
        {
            ...
            for (unsigned int i = 0; i < ctx->serviceCount; i++)
            {
                ...
                if (pipe_client_send_request(&svc->client, &req,
                                             &resp, 5000) != 0)
                {
                    LOG_W(LOG_TAG, "Reload config failed for '%s'",
                          svc->serviceName);
                }
            }
            LeaveCriticalSection(&ctx->lock);
        ...
        case IDM_INSTALL_SERVICE:
            LOG_I(LOG_TAG, "Menu: Install Service");
            {
                ...
                if (ShellExecuteExA(&sei)) {
                    ...
                    LOG_W(LOG_TAG, "Service installation failed or was cancelled (error %lu)",
                          GetLastError());
                }
            }
            break;
        ...
        default:
        {
            ...
            if (id >= IDM_SERVICE_BASE)
            {
                ...
                if (svcIdx < ctx->serviceCount)
                {
                    ...
                    if (instIdx < svc->instanceCount &&
                        svc->connected)
                    {
                        const char *instName =
                            svc->instances[instIdx].name;
                        ...
                        switch (action)
                        {
                            case IDM_INSTANCE_ACTION_START:
                                ...
                                LOG_I(LOG_TAG, "Menu: Start '%s' on '%s'",
                                      instName, svc->serviceName);
                                break;

                            case IDM_INSTANCE_ACTION_STOP:
                                cmd = PIPE_CMD_STOP_INSTANCE;
                                LOG_I(LOG_TAG, "Menu: Stop '%s' on '%s'",
                                      instName, svc->serviceName);
                                break;
                        ...
                        if (cmd != PIPE_CMD_COUNT)
                        {
                            ...
                            memset(&req, 0, sizeof(req));
                            req.command = cmd;
                            strncpy(req.instance_name, instName,
                                    sizeof(req.instance_name) - 1);
                            req.instance_name[
                                sizeof(req.instance_name) - 1] = '\0';

                            PipeResponse resp;
                            memset(&resp, 0, sizeof(resp));

                            if (pipe_client_send_request(
                                    &svc->client, &req,
                                    &resp, 5000) != 0)
                            {
                                LOG_W(LOG_TAG,
                                      "Action failed for '%s' on '%s'",
                                      instName, svc->serviceName);
                            }
                        }
                    }
