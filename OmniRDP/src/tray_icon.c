 ...
 */
static HICON
create_color_icon(COLORREF color)
{
	int size = 16;
	HDC hdc = GetDC(NULL);
...
push_listener_thread(LPVOID param)
{
	...
	while (ctx->running)
	{
		...
		for (unsigned int i = 0; i < ctx->serviceCount; i++)
		{
			...
			if (pipe_client_recv_push(&ctx->services[i].client, &pushType, &payload, &payloadLen) ==
			    0)
			{
				anyActivity = TRUE;
...
tray_icon_init(TrayAppCtx* ctx, HINSTANCE hInstance)
{
	...
	ctx->hwnd =
	    CreateWindowExA(0, HIDDEN_WIN_CLASS, "OmniRDP Tray", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
	...
	{
		LOG_W(LOG_TAG, "RegisterWindowMessageA(TaskbarCreated) failed: %lu", GetLastError());
...
tray_icon_update(TrayAppCtx* ctx)
{
	...
	_snprintf(nid.szTip, sizeof(nid.szTip), "OmniRDP: %u/%u running", runningCount, totalCount);
	nid.szTip[sizeof(nid.szTip) - 1] = '\0';
	...
	if (ctx->serviceCount > 0 && hServicesMenu)
	{
		for (unsigned int i = 0; i < ctx->serviceCount; i++)
		{
			...
			for (unsigned int j = 0; j < svc->instanceCount; j++)
			{
				...
				char instLabel[192];
				_snprintf(instLabel, sizeof(instLabel), "%s [%s]", inst->name, stateStr);
				instLabel[sizeof(instLabel) - 1] = '\0';
				AppendMenuA(hInstMenu, MF_STRING | MF_GRAYED | MF_DISABLED, 0, instLabel);

				/* Action items */
				UINT baseId =
				    IDM_SERVICE_BASE + (i * IDM_SERVICE_STRIDE) + (j * IDM_INSTANCE_SLOT_SIZE);

				if (inst->state == INSTANCE_RUNNING)
				{
					AppendMenuA(hInstMenu, MF_STRING, baseId + IDM_INSTANCE_ACTION_STOP, "Stop");
					AppendMenuA(hInstMenu, MF_STRING, baseId + IDM_INSTANCE_ACTION_RESTART,
					            "Restart");
				}
				else if (inst->state == INSTANCE_STOPPED)
				{
					AppendMenuA(hInstMenu, MF_STRING, baseId + IDM_INSTANCE_ACTION_START, "Start");
				}
				/* STARTING/RECONNECTING: no actionable items */

				/* Add instance submenu to the service menu */
				char instMenuText[192];
				_snprintf(instMenuText, sizeof(instMenuText), "%s [%s]", inst->name, stateStr);
				instMenuText[sizeof(instMenuText) - 1] = '\0';

				AppendMenuA(hSvcMenu, MF_POPUP | MF_STRING, (UINT_PTR)hInstMenu, instMenuText);
			}

			/* Add the service submenu to the services menu */
			AppendMenuA(hServicesMenu, MF_POPUP | MF_STRING, (UINT_PTR)hSvcMenu, svc->serviceName);
		}
	}
	else
	{
		/* No services found */
		AppendMenuA(hServicesMenu, MF_STRING | MF_GRAYED | MF_DISABLED, 0, "(No services found)");
	}
...
tray_icon_discover_services(TrayAppCtx* ctx)
{
	...
	BOOL ret = EnumServicesStatusExA(hSCM, SC_ENUM_PROCESS_INFO, SERVICE_WIN32_OWN_PROCESS,
	                                 SERVICE_STATE_ALL, NULL, 0, &bytesNeeded, &servicesReturned,
	                                 NULL, NULL);

	if (!ret && GetLastError() != ERROR_MORE_DATA)
	{
		LOG_W(LOG_TAG, "EnumServicesStatusExA (size query) failed: %lu", GetLastError());
	...
	LPENUM_SERVICE_STATUS_PROCESSA services =
	    (LPENUM_SERVICE_STATUS_PROCESSA)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, bytesNeeded);
	...
	if (!EnumServicesStatusExA(hSCM, SC_ENUM_PROCESS_INFO, SERVICE_WIN32_OWN_PROCESS,
	...
	for (DWORD i = 0; i < servicesReturned && ctx->serviceCount < 8; i++)
	{
		...
		{
			svc->connected = TRUE;
			LOG_I(LOG_TAG, "Connected to service '%s' via pipe '%s'", svc->serviceName,
			      svc->pipeName);
		}
		else
		{
			LOG_W(LOG_TAG, "Could not connect to service '%s' via pipe '%s'", svc->serviceName,
			      svc->pipeName);
		}
...
tray_icon_refresh_instances(TrayAppCtx* ctx)
{
	...
	for (unsigned int i = 0; i < ctx->serviceCount; i++)
	{
		...
		LOG_D("tray_icon", "refresh_instances: service[%u] '%s' connected=%d", i, svc->serviceName,
		      svc->connected);
		...
		LOG_D(LOG_TAG, "Refreshed %u instances for '%s'", svc->instanceCount, svc->serviceName);
	}
