...
int svc_config_write_template(const char *configPath) {
  ...
  fprintf(fp,
          ...
          "; Experimental backend RDPEGFX decode-only path. Default off and "
          "independent of viewer GFX.\n"
          ...
          ";   loadbalanceinfo:s:tsv://MS Terminal Services "
          "Plugin.1.Marketing_Pool\n"
          ...
          "; Experimental viewer RDPEGFX path. Default off. Uses canonical "
          "framebuffer snapshots, not backend GFX.\n"
          "viewer.gfx.enabled = false\n"
          "; viewer.gfx.codec = uncompressed|rfx|clearcodec; compressed "
          "codecs require compatible RDPEGFX capability negotiation and "
          "multimon QA.\n"
          "viewer.gfx.codec = uncompressed\n"
          "; Experimental viewer RFX encoder threading. Default off.\n"
          "viewer.gfx.rfx_threading_enabled = false\n"
          "; Viewer RDPEGFX dirty pacing limits. Defaults preserve one "
          "in-flight frame and a 4 MiB in-flight payload cap.\n"
          "viewer.gfx.dirty_max_in_flight_frames = 1\n"
          "viewer.gfx.dirty_max_in_flight_bytes = 4194304\n"
          "; Diagnostic only: force every viewer RDPEGFX incremental dirty "
          "update to full frame.\n"
          "viewer.gfx.diagnostic.full_frame_dirty = false\n"
          "; Default off. When enabled, slow classic viewers whose queued "
          "BitmapUpdate backlog exceeds either nonzero limit may have stale "
          "FIFO entries replaced with one latest framebuffer baseline.\n"
          ...
          "; NLA validates against the OmniRDP listener host/domain before "
          "OmniRDP can compare backend credentials. For backend-local "
          "accounts on another VM, use viewer.security.nla_enabled=false, "
          "viewer.security.tls_enabled=true, and "
