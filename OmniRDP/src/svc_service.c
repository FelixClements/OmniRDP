 ...
 */
static int
get_binary_path(const char* binary_name, char* out, size_t out_size)
{
 ...
 */
static int
set_config_file_acl(const char* filePath)
{
	DWORD dwRes;
	...
	/* Administrators SID */
	PSID pAdminSid = NULL;
	AllocateAndInitializeSid(&ntAuth, 2, SECURITY_BUILTIN_DOMAIN_RID, DOMAIN_ALIAS_RID_ADMINS, 0, 0,
	                         0, 0, 0, 0, &pAdminSid);
...
int
svc_config_write_template(const char* configPath)
{
 ...
 */
static void WINAPI
service_ctrl_handler(DWORD dwControl)
{
	if (!g_ctx)
		return;
...
int
svc_service_install(const char* serviceName, const char* configPath)
{
	...
	int ret = _snprintf(binaryPath, sizeof(binaryPath), "\"%s\" --service --service-name \"%s\"",
	...
	if (configPath && configPath[0] != '\0')
	{
		...
		ret = _snprintf(binaryPath + existing, sizeof(binaryPath) - existing, " --config \"%s\"",
		                configPath);
		...
		if (err == ERROR_SERVICE_MARKED_FOR_DELETE)
		{
			...
			for (int i = 0; i < 30; i++)
			{
				...
				schService = CreateServiceA(
				    ...
				    SERVICE_WIN32_OWN_PROCESS, SERVICE_AUTO_START, SERVICE_ERROR_NORMAL, binaryPath,
...
int
svc_service_uninstall(const char* serviceName)
{
	if (!serviceName || serviceName[0] == '\0')
	{
	...
	SC_HANDLE schService =
	    OpenServiceA(schSCManager, serviceName, SERVICE_STOP | DELETE | SERVICE_QUERY_STATUS);
	if (!schService)
	{
		DWORD err = GetLastError();
...
int
svc_service_start(const char* serviceName, const char* configPath)
{
	...
	ctx.statusHandle = RegisterServiceCtrlHandlerA(ctx.serviceName, service_ctrl_handler);
	if (!ctx.statusHandle)
	{
	...
	/* ── 5. Initialize logging (per-service log directory) ──── */
	{
		...
		if (svcCfg && svcCfg->log_dir[0] != '\0')
		{
			 ...
			 * isolation */
			snprintf(log_dir, sizeof(log_dir), "%s\\%s", svcCfg->log_dir, ctx.serviceName);
		}
		else
		{
	...
	LOG_I("svc_service_start", "OmniRDP service starting (serviceName=%s, configPath=%s)",
	      ctx.serviceName, ctx.configPath);

	/* ── 6. Find the OmniRDP.exe path ───────────────────────── */
	if (get_binary_path("OmniRDP.exe", ctx.exePath, sizeof(ctx.exePath)) != 0)
	{
		LOG_W("svc_service_start", "Failed to locate OmniRDP.exe; using fallback name");
...
int
svc_service_run_console(const char* serviceName, const char* configPath)
{
	OmniRDPSvcContext ctx;
	memset(&ctx, 0, sizeof(ctx));
	g_ctx = &ctx;
	...
	if (get_binary_path("OmniRDP.exe", ctx.exePath, sizeof(ctx.exePath)) != 0)
	{
		LOG_W("svc_service_run_console", "Failed to locate OmniRDP.exe; using fallback name");
		strncpy(ctx.exePath, "OmniRDP.exe", sizeof(ctx.exePath) - 1);
		ctx.exePath[sizeof(ctx.exePath) - 1] = '\0';
	}
