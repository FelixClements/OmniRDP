...
static void viewer_gfx_reset_graphics_pdu_reset(RDPGFX_RESET_GRAPHICS_PDU* reset_graphics)
{
    if (!reset_graphics)
        return;
...
static BOOL viewer_gfx_reset_graphics_pdu_copy(RDPGFX_RESET_GRAPHICS_PDU* destination,
...
static ViewerGfxEvent* viewer_gfx_event_new_surface_command(const RDPGFX_SURFACE_COMMAND* cmd)
{
...
static ViewerGfxEvent* viewer_gfx_event_new_simple(ViewerGfxEventType type, const void* payload,
                                                   size_t payload_size)
{
...
static void viewer_gfx_log_refcount_change(const char* owner, UINT32 frame_id, LONG old_count,
                                           LONG new_count)
{
...
static BOOL viewer_gfx_complete_frame_append_event(ViewerGfxCompleteFrame* frame,
...
static void viewer_gfx_log_frame_buffer_state_locked(const ViewerGfxFrameBuffer* buffer,
                                                     const char* reason)
{
...
static void viewer_gfx_frame_buffer_recompute_ids_locked(ViewerGfxFrameBuffer* buffer)
{
    UINT32 i = 0;
    UINT32 oldest = 0;
    UINT32 newest = 0;
...
static BOOL viewer_gfx_frame_buffer_begin_frame_locked(ViewerGfxFrameBuffer* buffer,
                                                       ...
                                                       ViewerGfxEvent* event)
{
    ...
    if (!viewer_gfx_complete_frame_append_event(frame, event))
    {
        ...
        WLog_ERR(TAG,
                 "Frame %" PRIu32
                 " capture failed; live forwarding continues but replay buffer unavailable",
                 start_frame->frameId);
        return FALSE;
    }
...
static BOOL viewer_gfx_frame_buffer_append_surface_command_locked(ViewerGfxFrameBuffer* buffer,
...
static BOOL viewer_gfx_frame_buffer_end_frame_locked(ViewerGfxFrameBuffer* buffer,
                                                     ...
                                                     ViewerGfxEvent* event)
{
    ...
    {
        WLog_ERR(TAG,
                 "Buffered frame %" PRIu32 " invalid or incomplete; evicting and using fallback refresh",
                 frame->frame_id);
        viewer_gfx_complete_frame_unref(frame);
        buffer->capture_frame = NULL;
        return FALSE;
    ...
    if (buffer->filled_slots >= VIEWER_GFX_FRAME_RING_CAPACITY)
    {
        ...
        BOOL golden_is_gfx = buffer->slots[0] && buffer->slots[0]->complete &&
            (buffer->slots[0]->codec_mask & (1ULL << RDPGFX_CODECID_CAPROGRESSIVE));
        UINT32 i;
        ...
        if (golden_is_gfx && (evict_slot == 0) && (best_non_gfx_slot != UINT32_MAX))
        {
            ...
            WLog_INFO(TAG,
                      "Protecting golden CAPROGRESSIVE frame in slot 0; evicting slot %" PRIu32 " instead",
                      best_non_gfx_slot);
        }
    ...
    if (frame->codec_mask & (1ULL << RDPGFX_CODECID_CAPROGRESSIVE))
    {
        ...
        if (slot_index != golden_slot)
        {
            ...
            BOOL current_is_gfx = current_golden &&
            ...
            else
            {
                     ...
                     frame->frame_id > current_golden->frame_id))
                {
                    ...
                    WLog_INFO(TAG,
                              "Swapped CAPROGRESSIVE frame %" PRIu32 " into golden slot 0",
                              frame->frame_id);
                }
            }
        }
...
static ViewerGfxCompleteFrame* viewer_gfx_frame_buffer_latest_locked(
    const ViewerGfxFrameBuffer* buffer)
{
    ...
    const ViewerGfxFrameBuffer* buffer, UINT32 minimum_frame_id)
{
    ...
    for (i = 0; i < VIEWER_GFX_FRAME_RING_CAPACITY; i++)
    {
         ...
         * fewer commands preferred, tie-break by larger payload. */
        {
            ...
            BOOL best_is_gfx = best && ((best->codec_mask & (1ULL << RDPGFX_CODECID_CAPROGRESSIVE)) != 0);
            BOOL frame_has_cc = (frame->codec_mask & (1ULL << RDPGFX_CODECID_CLEARCODEC)) != 0;
            BOOL best_has_cc = best && ((best->codec_mask & (1ULL << RDPGFX_CODECID_CLEARCODEC)) != 0);
            int better = 0;
...
static void viewer_gfx_set_join_state_locked(Viewer* viewer, ViewerJoinState state,
                                             ViewerJoinStrategy strategy, const char* reason)
{
    ...
    {
        WLog_INFO(TAG,
                  "Viewer %u join transition %s/%s -> %s/%s reason=%s targetFrame=%" PRIu32
                  " refreshGeneration=%" PRIu64,
...
static void viewer_gfx_begin_join_locked(Viewer* viewer, UINT64 now, const char* reason)
{
    if (!viewer)
        return;
...
static void viewer_gfx_finish_late_join_locked(Viewer* viewer, const char* reason)
{
    if (!viewer)
        return;
...
static BOOL viewer_gfx_viewer_accepts_live_locked(const ViewerGraphicsContext* gfx)
{
...
static BOOL viewer_gfx_drop_oldest_complete_frame_locked(ViewerGraphicsContext* gfx)
{
    UINT32 index = 0;
    UINT32 scanned = 0;
...
static BOOL viewer_gfx_enqueue_locked(ViewerGraphicsContext* gfx, ViewerGfxEvent* event)
{
...
static void viewer_gfx_publisher_state_reset_locked(ViewerGfxPublisherState* gfx)
{
    ...
    WLog_INFO(TAG, "Resetting publisher state: preserving surface map, clearing current frame and ring");
...
static ViewerGraphicsSurfaceState* viewer_gfx_find_surface_locked(ViewerGfxPublisherState* gfx,
                                                                  UINT16 surface_id)
{
    UINT32 i = 0;
...
static ViewerGraphicsSurfaceState* viewer_gfx_upsert_surface_locked(ViewerGfxPublisherState* gfx,
                                                                    UINT16 surface_id)
{
...
static void viewer_gfx_remove_surface_locked(ViewerGfxPublisherState* gfx, UINT16 surface_id)
{
...
static const RDPGFX_CAPSET* viewer_gfx_choose_caps(const ViewerGfxPublisherState* server_gfx,
...
static BOOL viewer_gfx_try_schedule_late_join_replay_locked(ViewerServer* server, Viewer* viewer,
                                                            UINT64 now)
{
    ...
    if (server->gfx.in_frame)
    {
        ...
        if (minimum_frame_id == server->gfx.current_frame_id)
        {
            ...
            WLog_DBG(TAG,
                     "Viewer %u try-schedule: frame %" PRIu32 " in progress, skipping it",
    ...
    if (recent_unsafe_codec_activity && (join_refresh_generation == 0))
    {
        ...
        {
            WLog_INFO(TAG,
                      "Viewer %u no replay-safe frames; attempting replay of latest frame %" PRIu32
                      " (unsafe codecs may cause partial decode)",
                                    ...
                                    ViewerGfxCompleteFrame* frame)
{
    ...
    viewer_gfx_set_join_state_locked(viewer, VIEWER_JOIN_STATE_REPLAYING,
                                     viewer->gfx.join_strategy, "sending replay baseline");
    started_ts = platform_get_timestamp_ms();
    viewer->gfx.join_start_ts = started_ts;
    LeaveCriticalSection(&viewer->gfx.lock);

    WLog_INFO(TAG, "Replaying frame %" PRIu32 " to viewer %u: %" PRIu32 " events, %" PRIu32
                    " commands",
              frame->frame_id, viewer->id, frame->event_count, frame->surface_command_count);
    WLog_INFO(TAG, "Viewer %u replay preamble: reset=%d activeSurfaces=%" PRIu32
...

    {
        ...
        for (i = 0; ok && (i < frame->event_count); i++)
        {
            ...
            WLog_INFO(TAG, "Viewer %u replay %" PRIu32 "/%" PRIu32 ": eventType=%u frameId=%" PRIu32,
                      viewer->id, i + 1U, frame->event_count, event->type, frame->frame_id);
            if (event->type == VIEWER_GFX_EVENT_SURFACE_COMMAND)
            {
                cmd_idx++;
                WLog_INFO(TAG, "Viewer %u replay command %" PRIu32 "/%" PRIu32
    ...
    if (server->backend)
    {
        ...
        WLog_INFO(TAG, "Viewer %u requested backend full refresh after replay cutover",
                  viewer->id);
    }
...
static UINT viewer_rdpgfx_caps_advertise(RdpgfxServerContext* context,
                                         const RDPGFX_CAPS_ADVERTISE_PDU* caps_advertise)
{
...
if (!selected)
    {
        WLog_WARN(TAG, "Viewer %u advertised incompatible RDPEGFX caps; staying on classic path",
                  viewer->id);
        EnterCriticalSection(&viewer->gfx.lock);
    ...
    {
        WLog_DBG(TAG,
                 "Viewer %u suppressing duplicate caps confirm (join_state=%s caps_ready=%d)",
    ...
    if (rc == CHANNEL_RC_OK)
    {
        ...
        if (viewer->activated && viewer_gfx_pending_activation_begins_rdpgfx_join(&viewer->gfx))
        {
            ...
            WLog_INFO(TAG,
                      "Viewer %u RDPEGFX caps confirmed after activation; gating live stream until replay/full refresh",
                      viewer->id);
        }
    }
    else
...
static UINT viewer_rdpgfx_frame_acknowledge(
    RdpgfxServerContext* context, const RDPGFX_FRAME_ACKNOWLEDGE_PDU* frame_acknowledge)
{
    Viewer* viewer = context ? (Viewer*)context->custom : NULL;
...
static BOOL viewer_pump_gfx(Viewer* viewer)
{
    ...
    for (;;)
    {
        ...
        if (!viewer_send_gfx_event(viewer, event))
        {
            LeaveCriticalSection(&viewer->send_lock);
            WLog_ERR(TAG,
                     "Viewer %u pump-gfx send failed (type=%u); disconnecting",
...
static BOOL viewer_server_publish_gfx_event(BackendClient* backend, ViewerGfxEvent* event)
{
    ...
    for (i = 0; i < MAX_VIEWERS; i++)
    {
        ...
        {
            WLog_DBG(TAG, "Viewer %u skipping live enqueue type=%u during late-join replay gate (join=%s)",
        ...
        if (enqueue && (event->type == VIEWER_GFX_EVENT_RESET_GRAPHICS))
        {
            ...
            {
                WLog_INFO(TAG,
                          "Viewer %u skipping ResetGraphics (activated %" PRIu64 " ms ago)",
                          viewer->id, elapsed);
                enqueue = FALSE;
            }
        }
...
static void viewer_get_backend_layout(BackendClient* backend, UINT32* width, UINT32* height,
                                      UINT32* generation)
{
...
static void viewer_server_clear_input_owner_locked(ViewerServer* server, UINT32 viewer_id,
                                                   const char* reason)
{
...
static BOOL viewer_input_owner_alive_locked(ViewerServer* server, UINT32 viewer_id)
{
...
static void viewer_graphics_context_reset(ViewerGraphicsContext* gfx, BackendClient* backend)
{
    UINT32 width = 0;
    UINT32 height = 0;
...
static BOOL viewer_send_frame_marker(Viewer* viewer, const SURFACE_FRAME_MARKER* marker)
{
    BOOL ret = FALSE;
    freerdp_peer* peer = viewer ? viewer->peer : NULL;
...
static BOOL can_viewer_send_input(Viewer* viewer)
{
    ...
    if (server->input_owner_active &&
        ...
        clear_reason = "disconnected, input ownership cleared";
             ...
             ((GetTickCount64() - server->input_owner_last_input_ts) >= INPUT_IDLE_TIMEOUT_MS))
    ...
    allowed = viewer_input_try_acquire(&state, viewer->id, viewer->connected, viewer->activated,
...
static ViewerGfxCompleteFrame* viewer_gfx_lookup_replay_frame(ViewerServer* server,
...
static BOOL viewer_gfx_send_surface_preamble(ViewerServer* server, Viewer* viewer)
{
    ViewerGfxEvent* reset_event = NULL;
    ViewerGfxEvent** create_events = NULL;
    ViewerGfxEvent** map_events = NULL;
    ...
    WLog_INFO(TAG, "Viewer %u sending surface preamble: reset=%d activeSurfaces=%" PRIu32
                    " mappedSurfaces=%" PRIu32,
              viewer->id, has_reset ? 1 : 0, active_surface_count, mapped_surface_count);
                                             ...
                                             const char* reason)
{
    ...
    {
        WLog_ERR(TAG, "Viewer %u direct bootstrap failed: surface preamble send error",
                 viewer->id);
        return FALSE;
    }
...
static BOOL viewer_gfx_step_join(ViewerServer* server, Viewer* viewer, UINT64 now)
{
            ...
            (refresh_state.completed_outcome != BACKEND_FULL_REFRESH_OUTCOME_NONE))
        {
            if (refresh_state.completed_outcome == BACKEND_FULL_REFRESH_OUTCOME_COMPLETED)
            {
                ...
                viewer_gfx_finish_late_join_locked(viewer,
                                                    "backend refresh completed; live stream enabled via surface preamble");
                LeaveCriticalSection(&viewer->gfx.lock);
                EnterCriticalSection(&viewer->send_lock);
                viewer->needs_full_refresh = FALSE;
                viewer->full_refresh_deadline_ts = 0;
                ...
                if (server->backend)
                {
                    (void)backend_request_full_refresh(server->backend);
                    WLog_INFO(TAG,
                              "Viewer %u requested follow-up backend refresh after preamble bootstrap",
                              viewer->id);
                }
                return TRUE;
            }
...
static BOOL on_extended_mouse_event(rdpInput* input, UINT16 flags, UINT16 x, UINT16 y)
{
    Viewer* viewer = find_viewer_by_peer(input->context->peer);
    if (!viewer || !can_viewer_send_input(viewer))
        return TRUE;
...
static DWORD WINAPI viewer_handle_peer(LPVOID arg)
{
    ...
    EnterCriticalSection(&viewer->gfx.lock);
        ...
        if (peer && peer->GetEventHandles)
        {
            ...
            if (wait_count == 0)
            {
                WLog_ERR(TAG, "Viewer %u failed to get FreeRDP transport event handles", viewer->id);
                break;
            }
        ...
        if (viewer->activated && viewer->gfx.vcm &&
            (viewer->gfx.vcm != INVALID_HANDLE_VALUE) && viewer->gfx.post_connect_complete &&
            WTSVirtualChannelManagerIsChannelJoined(viewer->gfx.vcm, DRDYNVC_SVC_CHANNEL_NAME))
        {
            ...
            gfx_event = viewer->gfx.rdpgfx ? rdpgfx_server_get_event_handle(viewer->gfx.rdpgfx)
                                           : NULL;
        }
        else
        {
        ...
        {
            EnterCriticalSection(&viewer->gfx.lock);
            if (viewer->gfx.rdpgfx && (rdpgfx_server_handle_messages(viewer->gfx.rdpgfx) != CHANNEL_RC_OK))
            {
                viewer_disable_rdpgfx_locked(viewer);
                LeaveCriticalSection(&viewer->gfx.lock);
                WLog_WARN(TAG,
                          "Viewer %u RDPEGFX message handling failed; falling back to classic path",
                          viewer->id);
                (void)viewer_gfx_enter_classic_fallback(g_viewer_server, viewer, now,
...
static BOOL peer_post_connect(freerdp_peer* peer)
{
    ...
    WLog_INFO(TAG, "Viewer %u peer_post_connect: GraphicsPipeline=%d",
              viewer->id, gfx_enabled);
    ...
    {
        BOOL backend_gfx = freerdp_settings_get_bool(
            g_viewer_server->backend->context->settings,
            FreeRDP_SupportGraphicsPipeline);
        if (!backend_gfx)
            gfx_enabled = FALSE;
 ...
 *   3. SupportDynamicTimeZone=TRUE so timezone data is consumed (fixes TPKT error)
 ...
 *   - CAPABILITIES_EXCHANGE_MONITOR_LAYOUT (needs MonitorCount/DefArray + SupportMonitorLayoutPdu)
 */
static BOOL peer_reached_state(freerdp_peer* peer, CONNECTION_STATE state)
{
    ...
    WLog_INFO(TAG, "peer_reached_state: Restoring server-side settings after GCC negotiation");
    WLog_INFO(TAG, "peer_reached_state:   Before: DesktopWidth=%"PRIu32", DesktopHeight=%"PRIu32", MonitorCount=%"PRIu32", SupportMonitorLayoutPdu=%s, SupportDynamicTimeZone=%s",
    ...
    for (i = 0; i < layout->monitor_count; i++)
    {
        ...
        WLog_INFO(TAG, "peer_reached_state:   monitor[%"PRIu32"]: x=%"PRId32", y=%"PRId32", width=%"PRId32", height=%"PRId32", is_primary=%s",
    ...
    WLog_INFO(TAG, "peer_reached_state:   After: DesktopWidth=%"PRIu32", DesktopHeight=%"PRIu32", MonitorCount=%"PRIu32", SupportMonitorLayoutPdu=TRUE, SupportDynamicTimeZone=TRUE",
              freerdp_settings_get_uint32(settings, FreeRDP_DesktopWidth),
              freerdp_settings_get_uint32(settings, FreeRDP_DesktopHeight),
              freerdp_settings_get_uint32(settings, FreeRDP_MonitorCount));
...
freerdp_settings_set_bool(settings, FreeRDP_SurfaceFrameMarkerEnabled, FALSE);
        ...
        WLog_INFO(TAG, "peer_accepted: SupportMonitorLayoutPdu=TRUE, desktop_width=%"PRIu32" > 1920=%s",
        ...
        if (desktop_width > 1920)
        {
            if (server && server->backend)
            {
                ...
                WLog_INFO(TAG, "peer_accepted: Configuring multi-monitor for viewer: MonitorCount=%"PRIu32,
                ...
                for (mi = 0; mi < layout->monitor_count; mi++)
                {
                    ...
                    WLog_INFO(TAG, "peer_accepted: viewer monitor[%"PRIu32"]: x=%"PRId32", y=%"PRId32", width=%"PRId32", height=%"PRId32", is_primary=%s, orig_screen=%"PRIu32,
...

                {
                    ...
                    UINT32 k = 0;
                    WLog_INFO(TAG, "peer_accepted: After setting MonitorDefArray: MonitorCount=%"PRIu32, monCountAfter);
                    for (k = 0; k < monCountAfter; k++)
                    {
                        ...
                        if (m)
                            WLog_INFO(TAG, "peer_accepted:   viewer settings[%"PRIu32"]: x=%"PRId32", y=%"PRId32", width=%"PRId32", height=%"PRId32", is_primary=%s",
        ...
        {
            rdpCertificate* cert = freerdp_certificate_new_from_file(platform_cert_path());
                                                ...
                                                UINT32 generation)
{
    ...
    for (int i = 0; i < MAX_VIEWERS; i++)
    {
        ...
        if (viewer->peer->context && viewer->peer->context->settings)
        {
            ...
            WLog_INFO(TAG, "viewer_server_notify_backend_layout_change: viewer[%d]: setting DesktopWidth=%"PRIu32", DesktopHeight=%"PRIu32,
                      i, width, height);
            ...
            if (width > 1920 && server && server->backend)
            {
                ...
                WLog_INFO(TAG, "viewer_server_notify_backend_layout_change: viewer[%d]: multi-monitor layout: MonitorCount=%"PRIu32,
                ...
                for (mi = 0; mi < layout->monitor_count; mi++)
                {
                    ...
                    WLog_INFO(TAG, "viewer_server_notify_backend_layout_change: viewer[%d]: monitor[%"PRIu32"]: x=%"PRId32", y=%"PRId32", width=%"PRId32", height=%"PRId32", is_primary=%s",
                              i, mi, mon.x, mon.y, mon.width, mon.height,
                              mon.is_primary ? "TRUE" : "FALSE");

                    freerdp_settings_set_pointer_array(vs,
                                                        FreeRDP_MonitorDefArray, mi, &mon);
                }
            }
        }
    }
    ...
    WLog_INFO(TAG,
              "Backend layout generation=%" PRIu32 " propagated to viewers as %ux%u; Backend layout change, queued full refresh for %u viewers",
    ...
    if (marker->frameAction == SURFACECMD_FRAMEACTION_END)
    {
        for (size_t i = 0; i < target_count; i++)
        {
            ...
            if (viewer->gfx.join_strategy == VIEWER_JOIN_STRATEGY_CLASSIC_FALLBACK)
                viewer_gfx_finish_late_join_locked(viewer, "classic fallback full refresh completed");
            LeaveCriticalSection(&viewer->gfx.lock);
        }
                                              ...
                                              const RDPGFX_RESET_GRAPHICS_PDU* reset_graphics)
{
    ...
    {
        WLog_ERR(TAG, "ResetGraphics copy failed; frame ring cleared and replay state invalidated");
        LeaveCriticalSection(&server->gfx.lock);
        viewer_gfx_event_unref(event);
        return FALSE;
    }
                                              ...
                                              const RDPGFX_CREATE_SURFACE_PDU* create_surface)
{
    ...
    WLog_INFO(TAG, "CreateSurface surfaceId=%" PRIu16 " size=%" PRIu16 "x%" PRIu16
                    " format=0x%04" PRIX16,
                                         ...
                                         const RDPGFX_END_FRAME_PDU* end_frame)
{
    ...
    if (viewer_gfx_frame_buffer_end_frame_locked(&server->gfx.frame_buffer, end_frame, event))
    {
        ...
        if (latest)
        {
            WLog_INFO(TAG, "Frame %" PRIu32 " complete: %" PRIu32 " events, %" PRIu32
    ...
    {
        WLog_INFO(TAG,
                  "Frame %" PRIu32 " completing backend refresh via GFX path",
                  end_frame->frameId);
        backend_mark_full_refresh_complete(server->backend);
    }
