...
static BOOL viewer_gfx_enter_classic_fallback(ViewerServer* server, Viewer* viewer, UINT64 now,
                                              const char* reason);
static BOOL viewer_gfx_request_backend_refresh(ViewerServer* server, Viewer* viewer, UINT64 now,
                                               const char* reason);
...
static BOOL
viewer_should_log_bitmap_perf(UINT64 batch_count, UINT64 publish_us, UINT32 send_failed_count)
{
...
viewer_classic_event_new(const BITMAP_UPDATE* bitmap)
{
	...
	event->bitmap->rectangles = (BITMAP_DATA*)calloc(bitmap->number, sizeof(BITMAP_DATA));
	...
	for (i = 0; i < bitmap->number; i++)
	{
		...
		if (bitmap->rectangles[i].bitmapLength > 0 && bitmap->rectangles[i].bitmapDataStream)
		{
			...
			memcpy(event->bitmap->rectangles[i].bitmapDataStream,
			       bitmap->rectangles[i].bitmapDataStream, bitmap->rectangles[i].bitmapLength);
		}
		else
		{
...
static void
viewer_classic_event_free(ViewerClassicEvent* event)
{
	UINT32 i = 0;
...
static void
viewer_surface_bits_event_free(ViewerSurfaceBitsEvent* event)
{
	if (!event)
		return;
...
static void
viewer_classic_queue_drop_oldest_locked(Viewer* viewer)
{
	...
	viewer->classic_queue_head = (viewer->classic_queue_head + 1) % VIEWER_CLASSIC_QUEUE_CAPACITY;
...
static void
viewer_classic_queue_clear_locked(Viewer* viewer)
{
...
static BOOL
viewer_classic_enqueue_locked(Viewer* viewer, const BITMAP_UPDATE* bitmap)
{
	ViewerClassicEvent* event = NULL;
	...
	{
		WLog_WARN(TAG, "Viewer %u classic queue full (%u entries), dropping oldest", viewer->id,
		          viewer->classic_queue_count);
		viewer_classic_queue_drop_oldest_locked(viewer);

		/* After dropping, mark viewer for full refresh to resync */
		viewer->needs_full_refresh = TRUE;
		viewer->full_refresh_deadline_ts = platform_get_timestamp_ms() + FULL_REFRESH_TIMEOUT_MS;
	}
	...
	viewer->classic_queue_tail = (viewer->classic_queue_tail + 1) % VIEWER_CLASSIC_QUEUE_CAPACITY;
	viewer->classic_queue_count++;
...
static BOOL
viewer_classic_enqueue_event_locked(Viewer* viewer, ViewerClassicEvent* event)
{
	if (!viewer || !event)
		return FALSE;
...
static ViewerClassicEvent*
viewer_classic_dequeue_locked(Viewer* viewer)
{
	ViewerClassicEvent* event = NULL;
 ...
 */
static void
viewer_surface_bits_queue_drop_oldest_locked(Viewer* viewer)
{
...
static void
viewer_surface_bits_queue_clear_locked(Viewer* viewer)
{
...
static BOOL
viewer_surface_bits_enqueue_event_locked(Viewer* viewer, ViewerSurfaceBitsEvent* event)
{
	if (!viewer || !event)
		return FALSE;
	...
	{
		WLog_WARN(TAG, "Viewer %u SurfaceBits queue full (%u entries), dropping oldest", viewer->id,
		          viewer->surface_bits_queue_count);
		viewer_surface_bits_queue_drop_oldest_locked(viewer);
...
static const char*
viewer_join_state_name(ViewerJoinState state)
{
	switch (state)
	{
...
static const char*
viewer_join_strategy_name(ViewerJoinStrategy strategy)
{
	switch (strategy)
	{
...
static BOOL
viewer_gfx_is_frame_event(ViewerGfxEventType type)
{
	return (type == VIEWER_GFX_EVENT_START_FRAME) || (type == VIEWER_GFX_EVENT_SURFACE_COMMAND) ||
...
viewer_gfx_event_alloc(ViewerGfxEventType type)
{
...
static ViewerGfxEvent*
viewer_gfx_event_new_reset_graphics(const RDPGFX_RESET_GRAPHICS_PDU* reset_graphics)
{
	ViewerGfxEvent* event = viewer_gfx_event_alloc(VIEWER_GFX_EVENT_RESET_GRAPHICS);
	if (!event)
		return NULL;

	if (!viewer_gfx_reset_graphics_pdu_copy(&event->u.reset_graphics, reset_graphics))
	{
		free(event);
		return NULL;
	}
...
{
	ViewerGfxEvent* event = viewer_gfx_event_alloc(VIEWER_GFX_EVENT_SURFACE_COMMAND);
	if (!event)
		return NULL;
...
static ViewerGfxEvent*
viewer_gfx_event_new_simple(ViewerGfxEventType type, const void* payload, size_t payload_size)
{
...
static void
viewer_gfx_event_ref(ViewerGfxEvent* event)
{
	if (event)
...
static void
viewer_gfx_event_unref(ViewerGfxEvent* event)
{
	if (!event)
		return;
...
static UINT64
viewer_gfx_event_payload_bytes(const ViewerGfxEvent* event)
{
	if (!event)
		return 0;
...
static void
viewer_gfx_log_refcount_change(const char* owner, UINT32 frame_id, LONG old_count, LONG new_count)
{
...
viewer_gfx_complete_frame_new(void)
{
	ViewerGfxCompleteFrame* frame = (ViewerGfxCompleteFrame*)calloc(1, sizeof(*frame));
	...
	frame->events = (ViewerGfxEvent**)calloc(VIEWER_GFX_MAX_FRAME_EVENTS, sizeof(ViewerGfxEvent*));
	if (!frame->events)
	{
		free(frame);
		return NULL;
...
static void
viewer_gfx_complete_frame_ref(ViewerGfxCompleteFrame* frame)
{
	LONG old_count = 0;
	LONG new_count = 0;
	...
	viewer_gfx_log_refcount_change("frame-ref", frame->frame_id, old_count, new_count);
}

static void
viewer_gfx_complete_frame_unref(ViewerGfxCompleteFrame* frame)
{
	LONG new_count = 0;
	LONG old_count = 0;
	UINT32 i = 0;
	...
	viewer_gfx_log_refcount_change("frame-unref", frame->frame_id, old_count, new_count);
	if (new_count != 0)
		return;

	WLog_INFO(TAG, "Freeing complete frame object for frameId=%" PRIu32, frame->frame_id);
...
static BOOL
viewer_gfx_complete_frame_append_event(ViewerGfxCompleteFrame* frame, ViewerGfxEvent* event)
{
	if (!frame || !event)
		return FALSE;
	...
	{
		WLog_ERR(TAG, "Frame %" PRIu32 " exceeded max events=%u; dropping incomplete capture",
...
static void
viewer_gfx_log_frame_buffer_state_locked(const ViewerGfxFrameBuffer* buffer, const char* reason)
{
	...
	WLog_INFO(TAG,
	          ...
	          reason ? reason : "unknown", buffer->filled_slots, VIEWER_GFX_FRAME_RING_CAPACITY,
	          buffer->next_slot, buffer->oldest_frame_id, buffer->newest_frame_id,
	          capture_frame_id);
}
...
static void
viewer_gfx_frame_buffer_init(ViewerGfxFrameBuffer* buffer)
{
...
static void
viewer_gfx_frame_buffer_reset_locked(ViewerGfxFrameBuffer* buffer)
{
	UINT32 i = 0;
	...
	if (buffer->capture_frame)
	{
		WLog_WARN(TAG, "Discarding incomplete frame frameId=%" PRIu32 " during frame ring reset",
	    ...
	    (buffer->slots[0]->codec_mask & (1ULL << RDPGFX_CODECID_CAPROGRESSIVE)))
	{
		...
		buffer->slots[0] = NULL;
		WLog_INFO(TAG, "Preserving golden CAPROGRESSIVE frame %" PRIu32 " across ring reset",
...
static void
viewer_gfx_frame_buffer_uninit(ViewerGfxFrameBuffer* buffer)
{
	if (!buffer || !buffer->initialized)
		return;
...
static BOOL
viewer_gfx_frame_buffer_begin_frame_locked(ViewerGfxFrameBuffer* buffer,
                                           ...
                                           ViewerGfxEvent* event)
{
	...
	WLog_INFO(TAG, "Allocating frame capture buffer for frameId=%" PRIu32, start_frame->frameId);
	frame = viewer_gfx_complete_frame_new();
	if (!frame)
	{
		WLog_ERR(TAG, "Failed to allocate frame capture buffer for frameId=%" PRIu32,
		         start_frame->frameId);
		return FALSE;
	}
	...
	WLog_INFO(TAG, "Allocated complete frame object for frameId=%" PRIu32, frame->frame_id);
...
static BOOL
viewer_gfx_frame_buffer_append_surface_command_locked(ViewerGfxFrameBuffer* buffer,
                                         ...
                                         ViewerGfxEvent* event)
{
	...
	if (buffer->filled_slots >= VIEWER_GFX_FRAME_RING_CAPACITY)
	{
		...
		if (golden_is_gfx && (evict_slot == 0) && (best_non_gfx_slot != UINT32_MAX))
		{
		...
		if (new_is_worse_than_all)
		{
			WLog_INFO(TAG,
			          ...
			          frame->frame_id, frame->surface_command_count, frame->total_payload_bytes);
			viewer_gfx_complete_frame_unref(frame);
		...
		WLog_INFO(TAG,
		          "Ring full: evicting frame %" PRIu32 " (score %d, %" PRIu32 " cmds, %" PRIu64
		          " bytes) from slot %" PRIu32,
	...
	WLog_INFO(TAG, "Adding frame %" PRIu32 " (%" PRIu64 " bytes) to ring slot %" PRIu32,
	...
	if (frame->codec_mask & (1ULL << RDPGFX_CODECID_CAPROGRESSIVE))
	{
		...
		if (slot_index != golden_slot)
		{
			...
			BOOL current_is_gfx = current_golden && (current_golden->codec_mask &
			                                         (1ULL << RDPGFX_CODECID_CAPROGRESSIVE));
			BOOL new_has_clearcodec =
			    (frame->codec_mask & (1ULL << RDPGFX_CODECID_CLEARCODEC)) != 0;
			BOOL golden_has_clearcodec =
			    current_golden &&
			    (current_golden->codec_mask & (1ULL << RDPGFX_CODECID_CLEARCODEC)) != 0;
			...
			{
				int new_score = (int)frame->surface_command_count;
				int golden_score = current_golden ? (int)current_golden->surface_command_count +
				                                        (current_is_gfx ? 0 : 10000)
				                                  : INT_MAX;

				if (new_score < golden_score ||
				    (new_score == golden_score && frame->frame_id > current_golden->frame_id))
				{
					buffer->slots[golden_slot] = frame;
					buffer->slots[slot_index] = current_golden;
					WLog_INFO(TAG, "Swapped CAPROGRESSIVE frame %" PRIu32 " into golden slot 0",
					          frame->frame_id);
				}
			}
		}
...
static ViewerGfxCompleteFrame*
viewer_gfx_frame_buffer_latest_replayable_locked(const ViewerGfxFrameBuffer* buffer,
                                                 UINT32 minimum_frame_id)
{
	...
	for (i = 0; i < VIEWER_GFX_FRAME_RING_CAPACITY; i++)
	{
		...
		if (!frame || !frame->complete || !frame->replay_safe ||
		    (frame->surface_command_count == 0) || (frame->frame_id < minimum_frame_id))
			continue;
		...
		if ((frame->surface_command_count < VIEWER_GFX_MIN_REPLAY_BASELINE_COMMANDS) &&
		...
		{
			BOOL frame_is_gfx = (frame->codec_mask & (1ULL << RDPGFX_CODECID_CAPROGRESSIVE)) != 0;
			BOOL best_is_gfx =
			    best && ((best->codec_mask & (1ULL << RDPGFX_CODECID_CAPROGRESSIVE)) != 0);
			BOOL frame_has_cc = (frame->codec_mask & (1ULL << RDPGFX_CODECID_CLEARCODEC)) != 0;
			BOOL best_has_cc =
			    best && ((best->codec_mask & (1ULL << RDPGFX_CODECID_CLEARCODEC)) != 0);
			int better = 0;
...
static void
viewer_gfx_set_join_state_locked(Viewer* viewer, ViewerJoinState state, ViewerJoinStrategy strategy,
                                 const char* reason)
{
...
static void
viewer_gfx_begin_join_locked(Viewer* viewer, UINT64 now, const char* reason)
{
	if (!viewer)
		return;
	...
	viewer_gfx_set_join_state_locked(viewer, VIEWER_JOIN_STATE_PENDING, VIEWER_JOIN_STRATEGY_NONE,
	                                 reason);
}

static void
viewer_gfx_finish_late_join_locked(Viewer* viewer, const char* reason)
{
	if (!viewer)
		return;
	...
	viewer_gfx_set_join_state_locked(viewer, VIEWER_JOIN_STATE_LIVE, VIEWER_JOIN_STRATEGY_NONE,
	                                 reason);
}
...
static void
viewer_gfx_queue_pop_head_locked(ViewerGraphicsContext* gfx)
{
	...
	if (event && (event->type == VIEWER_GFX_EVENT_END_FRAME) && (gfx->pending_frame_count > 0))
...
static BOOL
viewer_gfx_enqueue_locked(ViewerGraphicsContext* gfx, ViewerGfxEvent* event)
{
	...
	while ((gfx->queue_count >= VIEWER_GFX_QUEUE_CAPACITY) ||
	       (frame_event && (gfx->pending_frame_count >= VIEWER_GFX_MAX_PENDING_FRAMES)))
	{
...
static ViewerGfxEvent*
viewer_gfx_dequeue_locked(ViewerGraphicsContext* gfx)
{
	ViewerGfxEvent* event = NULL;
...
static void
viewer_gfx_queue_clear_locked(ViewerGraphicsContext* gfx)
{
...
static void
viewer_disable_rdpgfx_locked(Viewer* viewer)
{
	if (!viewer)
		return;
...
static void
viewer_gfx_publisher_state_init(ViewerGfxPublisherState* gfx)
{
...
static void
viewer_gfx_publisher_state_uninit(ViewerGfxPublisherState* gfx)
{
	if (!gfx || !gfx->initialized)
		return;
...
viewer_gfx_find_surface_locked(ViewerGfxPublisherState* gfx, UINT16 surface_id)
{
	UINT32 i = 0;
...
static ViewerGraphicsSurfaceState*
viewer_gfx_upsert_surface_locked(ViewerGfxPublisherState* gfx, UINT16 surface_id)
{
...
static void
viewer_gfx_remove_surface_locked(ViewerGfxPublisherState* gfx, UINT16 surface_id)
{
	ViewerGraphicsSurfaceState* surface = viewer_gfx_find_surface_locked(gfx, surface_id);
	if (surface)
...
static BOOL
viewer_gfx_try_schedule_late_join_replay_locked(ViewerServer* server, Viewer* viewer, UINT64 now)
{
	...
	if (server->gfx.in_frame)
	{
		...
		if (minimum_frame_id == server->gfx.current_frame_id)
		{
			...
			WLog_DBG(TAG, "Viewer %u try-schedule: frame %" PRIu32 " in progress, skipping it",
			         viewer->id, server->gfx.current_frame_id);
		}
		else if (minimum_frame_id > server->gfx.current_frame_id)
		{
			WLog_DBG(TAG,
			         "Viewer %u try-schedule: frame %" PRIu32 " in progress, minFid %" PRIu32
			         " already ahead",
	...
	WLog_INFO(TAG,
	          "Viewer %u try-schedule: ring scan minFid=%" PRIu32 " newest=%" PRIu32
	          " filled=%" PRIu32 "/%u inFrame=%d",
	          viewer->id, minimum_frame_id, newest_frame_id, server->gfx.frame_buffer.filled_slots,
	...
	latest = viewer_gfx_frame_buffer_latest_replayable_locked(&server->gfx.frame_buffer,
	                                                          minimum_frame_id);
	...
	if (recent_unsafe_codec_activity && (join_refresh_generation == 0))
	{
		...
		return viewer_gfx_bootstrap_direct_live(
		    server, viewer, now, "no frames in ring; bootstrapping via surface preamble only");
	}
	...
	switch (strategy)
	{
			...
			if (latest)
			{
				...
				if (viewer->gfx.join_refresh_generation != 0)
				{
					WLog_INFO(TAG,
					          ...
					          viewer->id, latest->frame_id, viewer->gfx.join_refresh_generation);
					viewer_gfx_set_join_state_locked(viewer, VIEWER_JOIN_STATE_WAIT_BACKEND_REFRESH,
				...
				viewer->gfx.join_start_ts = now;
				viewer->gfx.join_target_frame_id = latest->frame_id;
				viewer_gfx_set_join_state_locked(viewer, VIEWER_JOIN_STATE_PENDING, strategy,
				                                 "safe replay frame selected");
			}
			break;
		...
		case VIEWER_JOIN_STRATEGY_WAIT_NEXT_SAFE_FRAME:
			...
			{
				viewer_gfx_set_join_state_locked(viewer, VIEWER_JOIN_STATE_WAIT_NEXT_SAFE_FRAME,
				                                 strategy, "waiting for next replay-safe frame");
			}
			break;
...
static BOOL
viewer_gfx_replay_frame(ViewerServer* server, Viewer* viewer, ViewerGfxCompleteFrame* frame)
{
	...
	if (!server || !viewer || !frame || !frame->complete || (frame->event_count < 2))
		return FALSE;

	EnterCriticalSection(&server->gfx.lock);
	if (server->gfx.has_latest_reset_graphics)
	{
		reset_event = viewer_gfx_event_new_reset_graphics(&server->gfx.latest_reset_graphics);
		if (!reset_event)
			ok = FALSE;
		has_reset = TRUE;
	}
	...
	if (ok)
	{
		...
		{
			create_events = (ViewerGfxEvent**)calloc(active_surface_count, sizeof(ViewerGfxEvent*));
			if (!create_events)
				ok = FALSE;
		}
		...
		{
			map_events = (ViewerGfxEvent**)calloc(mapped_surface_count, sizeof(ViewerGfxEvent*));
			if (!map_events)
				ok = FALSE;
		}
	}
	...
	if (ok)
	{
		for (i = 0; i < VIEWER_GFX_MAX_ACTIVE_SURFACES; i++)
		{
			...
			map_events[map_index] = viewer_gfx_event_new_simple(
			    VIEWER_GFX_EVENT_MAP_SURFACE_TO_OUTPUT, &surface->map_surface_to_output,
	...
	if (!ok)
	{
		WLog_WARN(TAG, "Viewer %u replay scheduling failed; falling back to backend refresh",
		          viewer->id);
		goto cleanup;
	}
	...
	viewer_gfx_set_join_state_locked(viewer, VIEWER_JOIN_STATE_REPLAYING, viewer->gfx.join_strategy,
	...
	WLog_INFO(TAG,
	          "Replaying frame %" PRIu32 " to viewer %u: %" PRIu32 " events, %" PRIu32 " commands",
	          frame->frame_id, viewer->id, frame->event_count, frame->surface_command_count);
	WLog_INFO(TAG,
	          "Viewer %u replay preamble: reset=%d activeSurfaces=%" PRIu32
	          " mappedSurfaces=%" PRIu32,
	          viewer->id, has_reset ? 1 : 0, active_surface_count, mapped_surface_count);
...

	{
		...
		for (i = 0; ok && (i < frame->event_count); i++)
		{
			...
			WLog_INFO(TAG,
			          "Viewer %u replay %" PRIu32 "/%" PRIu32 ": eventType=%u frameId=%" PRIu32,
			          viewer->id, i + 1U, frame->event_count, event->type, frame->frame_id);
			if (event->type == VIEWER_GFX_EVENT_SURFACE_COMMAND)
			{
				cmd_idx++;
				WLog_INFO(TAG,
				          "Viewer %u replay command %" PRIu32 "/%" PRIu32 ": surfaceId=%" PRIu16
				          " codecId=%" PRIu16 " length=%" PRIu32,
				          viewer->id, cmd_idx, frame->surface_command_count,
				          event->u.surface_command.surfaceId, event->u.surface_command.codecId,
			...
			{
				WLog_ERR(TAG,
				         "Viewer %u replay failed at event %" PRIu32 "/%" PRIu32 "; disconnecting",
	...
	if (!ok)
	{
		...
		WLog_ERR(TAG, "Viewer %u replay failed: queue/send error, disconnecting", viewer->id);
		goto cleanup;
	}
	...
	viewer_gfx_finish_late_join_locked(viewer, "replay baseline sent; switching to live stream");
	LeaveCriticalSection(&viewer->gfx.lock);
	WLog_INFO(TAG, "Frame %" PRIu32 " replayed to viewer %u in %" PRIu64 " ms", frame->frame_id,
	          viewer->id, elapsed_ms);
	WLog_INFO(TAG, "Viewer %u replay baseline frame=%" PRIu32 " complete; live stream enabled",
	          viewer->id, frame->frame_id);

	if (server->backend)
	{
		(void)backend_request_full_refresh(server->backend);
		WLog_INFO(TAG, "Viewer %u requested backend full refresh after replay cutover", viewer->id);
	}
                             ...
                             const RDPGFX_CAPS_ADVERTISE_PDU* caps_advertise)
{
	...
	if (!selected)
	{
		WLog_WARN(TAG,
		          ...
		          viewer->id);
		EnterCriticalSection(&viewer->gfx.lock);
		if (viewer->gfx.negotiation_outcome == VIEWER_GFX_NEGOTIATION_CLASSIC_FALLBACK)
		{
			LeaveCriticalSection(&viewer->gfx.lock);
		...
		LeaveCriticalSection(&viewer->gfx.lock);
		if (viewer->activated)
			(void)viewer_gfx_enter_classic_fallback(server, viewer, platform_get_timestamp_ms(),
	...
	if (caps_ready_was && use_rdpgfx_was)
	{
		WLog_DBG(TAG,
		         ...
		         "caps_ready=%d)",
		         viewer->id, viewer_join_state_name(viewer->gfx.join_state), caps_ready_was);
		LeaveCriticalSection(&viewer->gfx.lock);
		return CHANNEL_RC_OK;
	}
	LeaveCriticalSection(&viewer->gfx.lock);
	...
	if (rc == CHANNEL_RC_OK)
	{
		...
		{
			WLog_INFO(TAG, "Viewer %u RDPEGFX caps confirm progressed negotiation", viewer->id);
		}

		if (viewer->activated && viewer_gfx_pending_activation_begins_rdpgfx_join(&viewer->gfx))
		{
...
static UINT
viewer_rdpgfx_frame_acknowledge(RdpgfxServerContext* context,
...
static BOOL
viewer_send_gfx_event(Viewer* viewer, ViewerGfxEvent* event)
{
	UINT status = CHANNEL_RC_OK;
	freerdp_peer* peer = viewer ? viewer->peer : NULL;

	if (!viewer || !event || !peer || !viewer->gfx.rdpgfx || !viewer->gfx.use_rdpgfx)
		return FALSE;
	...
	switch (event->type)
	{
		case VIEWER_GFX_EVENT_RESET_GRAPHICS:
			status =
			    viewer->gfx.rdpgfx->ResetGraphics(viewer->gfx.rdpgfx, &event->u.reset_graphics);
			break;

		case VIEWER_GFX_EVENT_CREATE_SURFACE:
			status =
			    viewer->gfx.rdpgfx->CreateSurface(viewer->gfx.rdpgfx, &event->u.create_surface);
			break;

		case VIEWER_GFX_EVENT_DELETE_SURFACE:
			status =
			    viewer->gfx.rdpgfx->DeleteSurface(viewer->gfx.rdpgfx, &event->u.delete_surface);
			break;

		case VIEWER_GFX_EVENT_MAP_SURFACE_TO_OUTPUT:
			status = viewer->gfx.rdpgfx->MapSurfaceToOutput(viewer->gfx.rdpgfx,
			                                                &event->u.map_surface_to_output);
			break;

		case VIEWER_GFX_EVENT_START_FRAME:
			status = viewer->gfx.rdpgfx->StartFrame(viewer->gfx.rdpgfx, &event->u.start_frame);
			break;

		case VIEWER_GFX_EVENT_SURFACE_COMMAND:
			status =
			    viewer->gfx.rdpgfx->SurfaceCommand(viewer->gfx.rdpgfx, &event->u.surface_command);
			break;

		case VIEWER_GFX_EVENT_END_FRAME:
			status = viewer->gfx.rdpgfx->EndFrame(viewer->gfx.rdpgfx, &event->u.end_frame);
			break;

		case VIEWER_GFX_EVENT_DELETE_ENCODING_CONTEXT:
			status = viewer->gfx.rdpgfx->DeleteEncodingContext(viewer->gfx.rdpgfx,
			                                                   &event->u.delete_encoding_context);
			break;
	}
...
viewer_pump_gfx(Viewer* viewer)
{
	...
	gated = (viewer->gfx.join_state != VIEWER_JOIN_STATE_LIVE) && (viewer->gfx.queue_count > 0);
	if (gated)
	{
		WLog_DBG(TAG, "Viewer %u pump-gfx: %" PRIu32 " events queued but join_state=%s (gated)",
	...
	for (;;)
	{
		...
		{
			LeaveCriticalSection(&viewer->send_lock);
			WLog_ERR(TAG, "Viewer %u pump-gfx send failed (type=%u); disconnecting", viewer->id,
			         event->type);
			viewer_gfx_event_unref(event);
			return FALSE;
		}
...
static BOOL
viewer_pump_classic(Viewer* viewer)
{
	ViewerClassicEvent* event = NULL;
	UINT32 pumped = 0;
	UINT32 coalesced = 0;
	...
	for (;;)
	{
		...
		while (viewer->classic_queue_count > 0)
		{
			...
			new_rects =
			    (BITMAP_DATA*)realloc(event->bitmap->rectangles,
	...
	{
		WLog_INFO(TAG,
		          "Viewer %u pump-classic: delivered %" PRIu32 " bitmaps (coalesced=%" PRIu32
		          ") %" PRIu32 " SurfaceBits",
...
static BOOL
viewer_server_publish_gfx_event(BackendClient* backend, ViewerGfxEvent* event)
{
	ViewerServer* server = g_viewer_server;
	BOOL sent_any = FALSE;
	int i = 0;
	...
	for (i = 0; i < MAX_VIEWERS; i++)
	{
		...
		if (!viewer->peer || !viewer->connected || !viewer->activated || viewer->stop_requested)
			continue;

		EnterCriticalSection(&viewer->gfx.lock);
		enqueue = !viewer->gfx.rdpgfx_temporarily_disabled && viewer->gfx.channel_opened &&
		          viewer->gfx.caps_ready && viewer_gfx_negotiation_is_rdpgfx_ready(&viewer->gfx) &&
		          viewer_gfx_viewer_accepts_live_locked(&viewer->gfx);
		join_state = viewer->gfx.join_state;
		if (!enqueue && !viewer->gfx.rdpgfx_temporarily_disabled && viewer->gfx.channel_opened &&
		    viewer->gfx.caps_ready && viewer_gfx_negotiation_is_rdpgfx_ready(&viewer->gfx) &&
		    (viewer->gfx.join_state != VIEWER_JOIN_STATE_LIVE))
		{
			WLog_DBG(TAG,
			         ...
			         "replay gate (join=%s)",
			         viewer->id, event->type, viewer_join_state_name(viewer->gfx.join_state));
		}
		waiting_for_ack = (viewer->gfx.join_state == VIEWER_JOIN_STATE_WAIT_REPLAY_ACK);
		if (enqueue && (event->type == VIEWER_GFX_EVENT_RESET_GRAPHICS))
		{
			UINT64 elapsed = platform_get_timestamp_ms() - viewer->gfx.last_activated_ts;
			if (elapsed < 5000)
			{
				WLog_INFO(TAG, "Viewer %u skipping ResetGraphics (activated %" PRIu64 " ms ago)",
				          viewer->id, elapsed);
				enqueue = FALSE;
			}
		}
		if (enqueue)
		{
			...
			if (!viewer_gfx_enqueue_locked(&viewer->gfx, event))
			{
				...
				{
					WLog_ERR(TAG,
					         "Viewer %u queue overflow (count=%" PRIu32 " pending=%" PRIu32
					         "); disconnecting",
					         viewer->id, viewer->gfx.queue_count, viewer->gfx.pending_frame_count);
					viewer->stop_requested = TRUE;
				}
				viewer_gfx_event_unref(event);
			}
	...
	if (event->type == VIEWER_GFX_EVENT_END_FRAME)
	{
		WLog_INFO(TAG, "GFX publish endFrame fid=%" PRIu32 " sent=%d", event->u.end_frame.frameId,
		          sent_any);
	}
...
static void
viewer_get_backend_layout(BackendClient* backend, UINT32* width, UINT32* height, UINT32* generation)
{
	...
	if (backend)
	{
		backend_get_desktop_layout(backend, &current_width, &current_height, &current_generation);
		if (current_width == 0)
...
static void
viewer_server_clear_input_owner_locked(ViewerServer* server, UINT32 viewer_id, const char* reason)
{
	if (!server || !server->input_owner_active || (server->input_owner_viewer_id != viewer_id))
		return;
...
static BOOL
viewer_input_owner_alive_locked(ViewerServer* server, UINT32 viewer_id)
{
...
static void
viewer_graphics_context_init(ViewerGraphicsContext* gfx)
{
	if (!gfx || gfx->initialized)
		return;
...
static void
viewer_graphics_context_reset(ViewerGraphicsContext* gfx, BackendClient* backend)
{
	UINT32 width = 0;
	UINT32 height = 0;
...
static void
viewer_graphics_context_uninit(ViewerGraphicsContext* gfx)
{
	if (!gfx || !gfx->initialized)
		return;
...
static BOOL
viewer_send_state_init(Viewer* viewer)
{
	if (!viewer)
		return FALSE;
...
static void
viewer_send_state_uninit(Viewer* viewer)
{
...
static void
viewer_join_thread(Viewer* viewer)
{
...
static void
viewer_cleanup_slot(Viewer* viewer)
{
	if (!viewer)
		return;
...
static void
viewer_pointer_shape_entry_reset(PointerShapeEntry* shape)
{
	if (!shape)
		return;
...
static BOOL
viewer_pointer_shape_entry_copy(PointerShapeEntry* destination, const PointerShapeEntry* source)
{
	if (!destination || !source)
		return FALSE;
	...
	*destination = *source;
	...
	if (source->xorMaskLength > 0)
	{
		...
		memcpy(destination->xorMaskData, source->xorMaskData, source->xorMaskLength);
	}
	...
	if (source->andMaskLength > 0)
	{
		...
		memcpy(destination->andMaskData, source->andMaskData, source->andMaskLength);
	}
...
static BOOL
viewer_forward_pointer(Viewer* viewer, BOOL force)
{
	ViewerServer* server = g_viewer_server;
	freerdp_peer* peer = viewer ? viewer->peer : NULL;
	...
	if (!viewer || !server || !backend || !peer || !peer->context || !peer->context->update ||
	    !peer->context->update->pointer || !viewer->connected || !viewer->activated ||
	    !peer->activated)
		return FALSE;

	backend_get_pointer_snapshot(backend, &pointer_x, &pointer_y, &pointer_visible, &pointer_type,
	                             &active_shape, &position_generation, &shape_generation);
	if (active_shape && !viewer_pointer_shape_entry_copy(&shape_copy, active_shape))
		return FALSE;

	shape_changed = force || (shape_generation != viewer->last_pointer_shape_generation);
	position_changed = force || (position_generation != viewer->last_pointer_position_generation);
	...
	if (send_shape)
	{
		...
		{
			IFCALLRET(peer->context->update->pointer->PointerSystem, sent, peer->context,
			          &pointer_system);
		}
		else if ((shape_copy.xorBpp > 0) && peer->context->update->pointer->PointerNew)
		{
	...
	if (sent && send_position && peer->context->update->pointer->PointerPosition)
		IFCALLRET(peer->context->update->pointer->PointerPosition, sent, peer->context,
		          &pointer_position);
	rdp_update_unlock(peer->context->update);
...
static BOOL
viewer_send_surface_bits(Viewer* viewer, const SURFACE_BITS_COMMAND* cmd)
{
	BOOL ret = FALSE;
	freerdp_peer* peer = viewer ? viewer->peer : NULL;
...
static BOOL
viewer_send_bitmap_update(Viewer* viewer, const BITMAP_UPDATE* bitmap)
{
	BOOL ret = FALSE;
	freerdp_peer* peer = viewer ? viewer->peer : NULL;
	UINT64 send_started_us = 0;
...
static BOOL
viewer_send_bitmap_update_locked(Viewer* viewer, const BITMAP_UPDATE* bitmap)
{
	BOOL ret = FALSE;
	freerdp_peer* peer = viewer ? viewer->peer : NULL;
	UINT64 send_started_us = 0;
...
static BOOL
viewer_send_frame_marker(Viewer* viewer, const SURFACE_FRAME_MARKER* marker)
{
	BOOL ret = FALSE;
	freerdp_peer* peer = viewer ? viewer->peer : NULL;
	...
	rdp_update_lock(peer->context->update);
	IFCALLRET(peer->context->update->SurfaceFrameMarker, ret, peer->context, marker);
	rdp_update_unlock(peer->context->update);
	if (ret)
		viewer->packets_sent++;
	else
...
static Viewer*
find_viewer_by_peer(freerdp_peer* peer)
{
	ViewerServer* server = g_viewer_server;
	if (!server)
		return NULL;
...
static BOOL
can_viewer_send_input(Viewer* viewer)
{
	ViewerServer* server = g_viewer_server;
	...
	if (clear_reason)
		viewer_server_clear_input_owner_locked(server, server->input_owner_viewer_id, clear_reason);
	...
	allowed =
	    viewer_input_try_acquire(&state, viewer->id, viewer->connected, viewer->activated,
...
static BOOL
viewer_gfx_handshake_ready_locked(const Viewer* viewer)
{
	return viewer && viewer->activated && viewer->gfx.post_connect_complete &&
	       (viewer->gfx.drdynvc_state == DRDYNVC_STATE_READY) && viewer->gfx.channel_opened &&
	       viewer->gfx.caps_ready && viewer_gfx_negotiation_is_rdpgfx_ready(&viewer->gfx) &&
...
viewer_gfx_lookup_replay_frame(ViewerServer* server, UINT32 minimum_frame_id)
{
	...
	EnterCriticalSection(&server->gfx.lock);
	frame = viewer_gfx_frame_buffer_latest_replayable_locked(&server->gfx.frame_buffer,
	                                                         minimum_frame_id);
	LeaveCriticalSection(&server->gfx.lock);
	return frame;
}
...
static BOOL
viewer_gfx_request_backend_refresh(ViewerServer* server, Viewer* viewer, UINT64 now,
                                   const char* reason)
{
	...
	WLog_INFO(TAG, "Viewer %u late join using backend refresh generation=%" PRIu64 " reason=%s",
	          viewer->id, generation, reason ? reason : "unspecified");
	return TRUE;
}

static BOOL
viewer_gfx_enter_classic_fallback(ViewerServer* server, Viewer* viewer, UINT64 now,
                                  const char* reason)
{
	if (!viewer)
		return FALSE;
	...
	viewer->gfx.join_start_ts = now;
	viewer_gfx_set_join_state_locked(viewer, VIEWER_JOIN_STATE_PENDING,
	                                 VIEWER_JOIN_STRATEGY_CLASSIC_FALLBACK, reason);
	LeaveCriticalSection(&viewer->gfx.lock);
	...
	WLog_WARN(TAG, "Viewer %u late join falling back to classic path reason=%s", viewer->id,
	          reason ? reason : "unspecified");
	return TRUE;
}

static void
viewer_gfx_reject_join(Viewer* viewer, const char* reason)
{
	if (!viewer)
		return;
	...
	LeaveCriticalSection(&viewer->gfx.lock);
	viewer->stop_requested = TRUE;
	WLog_ERR(TAG, "Viewer %u join rejected reason=%s", viewer->id, reason ? reason : "unspecified");
}

static BOOL
viewer_gfx_send_surface_preamble(ViewerServer* server, Viewer* viewer)
{
	ViewerGfxEvent* reset_event = NULL;
	ViewerGfxEvent** create_events = NULL;
	ViewerGfxEvent** map_events = NULL;
...
static BOOL
viewer_gfx_bootstrap_direct_live(ViewerServer* server, Viewer* viewer, UINT64 now,
                                 const char* reason)
{
	if (!server || !viewer)
		return FALSE;

	WLog_INFO(TAG, "Viewer %u bootstrapping directly to LIVE via surface preamble reason=%s",
	          viewer->id, reason ? reason : "unspecified");

	if (!viewer_gfx_send_surface_preamble(server, viewer))
	{
		WLog_ERR(TAG, "Viewer %u direct bootstrap failed: surface preamble send error", viewer->id);
		return FALSE;
	}
	...
	{
		(void)backend_request_full_refresh(server->backend);
		WLog_INFO(TAG, "Viewer %u requested backend full refresh after direct bootstrap",
		          viewer->id);
	}
...
static BOOL
viewer_gfx_step_join(ViewerServer* server, Viewer* viewer, UINT64 now)
{
	...
	{
		if (viewer_late_join_timeout_fallback_due(VIEWER_JOIN_STRATEGY_REPLAY_SAFE_FRAME,
		                                          viewer->gfx.join_start_ts, now,
		                                          VIEWER_WAIT_REPLAY_ACK_TIMEOUT_MS, TRUE))
		{
			...
			return viewer_gfx_enter_classic_fallback(server, viewer, now,
			                                         "replay ack timeout after backend refresh");
		}
		return TRUE;
	}

	if ((state == VIEWER_JOIN_STATE_PENDING) || (state == VIEWER_JOIN_STATE_WAIT_NEXT_SAFE_FRAME) ||
	    (state == VIEWER_JOIN_STATE_WAIT_BACKEND_REFRESH))
	{
		...
		target_frame_id = viewer->gfx.join_target_frame_id;
		refresh_generation = viewer->gfx.join_refresh_generation;
		if ((state == VIEWER_JOIN_STATE_PENDING) && (strategy == VIEWER_JOIN_STRATEGY_NONE))
		{
			if (!viewer_gfx_handshake_ready_locked(viewer))
			{
				LeaveCriticalSection(&viewer->gfx.lock);
	...
	if ((strategy == VIEWER_JOIN_STRATEGY_REPLAY_SAFE_FRAME) && (target_frame_id != 0))
	{
		...
		if (!frame)
		{
			if (refresh_generation == 0)
				return viewer_gfx_request_backend_refresh(server, viewer, now,
				                                          "selected replay frame unavailable");
			return viewer_gfx_enter_classic_fallback(
			    server, viewer, now, "selected replay frame unavailable after backend refresh");
		}
	...
	{
		if (viewer_late_join_timeout_fallback_due(VIEWER_JOIN_STRATEGY_WAIT_NEXT_SAFE_FRAME,
		                                          viewer->gfx.join_start_ts, now,
		                                          FULL_REFRESH_TIMEOUT_MS, FALSE))
		{
			...
			if (refresh_generation == 0)
				return viewer_gfx_bootstrap_direct_live(server, viewer, now,
				                                        "no replay baseline after timeout");
			return viewer_gfx_enter_classic_fallback(
			    server, viewer, now, "wait-next-safe-frame timeout after backend refresh");
		}
		return TRUE;
	}
	...
	{
		if (backend_abandon_full_refresh_if_timed_out(server->backend, refresh_generation,
		...
		if ((refresh_state.completed_generation >= refresh_generation) &&
		    (refresh_state.completed_outcome != BACKEND_FULL_REFRESH_OUTCOME_NONE))
		{
			if (refresh_state.completed_outcome == BACKEND_FULL_REFRESH_OUTCOME_COMPLETED)
			{
				WLog_INFO(TAG,
				...
				{
					return viewer_gfx_enter_classic_fallback(
					    server, viewer, now, "surface preamble failed after backend refresh");
				}
				EnterCriticalSection(&viewer->gfx.lock);
			...
			return viewer_gfx_enter_classic_fallback(server, viewer, now,
			                                         "backend refresh failed or timed out");
		}
	}

	if (strategy == VIEWER_JOIN_STRATEGY_CLASSIC_FALLBACK)
		return viewer_gfx_enter_classic_fallback(server, viewer, now, "caps or handshake fallback");

	return TRUE;
}

static BOOL
on_mouse_event(rdpInput* input, UINT16 flags, UINT16 x, UINT16 y)
{
	Viewer* viewer = find_viewer_by_peer(input->context->peer);
	if (!viewer || !can_viewer_send_input(viewer))
		return TRUE;

	freerdp_input_send_mouse_event(g_viewer_server->backend->context->input, flags, x, y);
	backend_store_pointer_position(g_viewer_server->backend, x, y);
	return TRUE;
}

static BOOL
on_extended_mouse_event(rdpInput* input, UINT16 flags, UINT16 x, UINT16 y)
{
	Viewer* viewer = find_viewer_by_peer(input->context->peer);
	if (!viewer || !can_viewer_send_input(viewer))
		return TRUE;

	freerdp_input_send_extended_mouse_event(g_viewer_server->backend->context->input, flags, x, y);
	backend_store_pointer_position(g_viewer_server->backend, x, y);
	return TRUE;
}

static BOOL
on_keyboard_event(rdpInput* input, UINT16 flags, UINT8 code)
{
	Viewer* viewer = find_viewer_by_peer(input->context->peer);
	if (!viewer || !can_viewer_send_input(viewer))
		return TRUE;

	freerdp_input_send_keyboard_event(g_viewer_server->backend->context->input, flags, code);
	return TRUE;
}

static DWORD WINAPI
viewer_handle_peer(LPVOID arg)
{
	...
	while (!viewer->stop_requested)
	{
		    ...
		    (now >= viewer->full_refresh_deadline_ts))
		{
			...
			{
				LeaveCriticalSection(&viewer->send_lock);
				viewer_gfx_reject_join(viewer, "classic fallback full refresh timed out");
				break;
			}
		}
		LeaveCriticalSection(&viewer->send_lock);
		...
		EnterCriticalSection(&viewer->gfx.lock);
		if (viewer_gfx_pending_activation_timeout_due(&viewer->gfx, now,
		                                              VIEWER_RDPEGFX_NEGOTIATION_TIMEOUT_MS))
		{
			LeaveCriticalSection(&viewer->gfx.lock);
		...
		if (g_viewer_server->slow_viewer_disconnect_enabled &&
		    viewer_disconnect_due(viewer, g_viewer_server->slow_viewer_disconnect_ms, now))
			break;
		...
		{
			wait_count = peer->GetEventHandles(peer, wait_objects, MAXIMUM_WAIT_OBJECTS);
			if (wait_count == 0)
			{
		...
		EnterCriticalSection(&viewer->gfx.lock);
		if (viewer->activated && viewer->gfx.vcm && (viewer->gfx.vcm != INVALID_HANDLE_VALUE) &&
		    viewer->gfx.post_connect_complete &&
		    WTSVirtualChannelManagerIsChannelJoined(viewer->gfx.vcm, DRDYNVC_SVC_CHANNEL_NAME))
		{
			...
			if ((drdynvc_state == DRDYNVC_STATE_READY) && viewer->gfx.rdpgfx &&
			    !viewer->gfx.channel_opened && !viewer->gfx.rdpgfx_temporarily_disabled)
			{
			...
			gfx_event =
			    viewer->gfx.rdpgfx ? rdpgfx_server_get_event_handle(viewer->gfx.rdpgfx) : NULL;
		}
		else
		{
			gfx_event = NULL;
		...
		{
			EnterCriticalSection(&viewer->gfx.lock);
			if (viewer->gfx.rdpgfx &&
			    (rdpgfx_server_handle_messages(viewer->gfx.rdpgfx) != CHANNEL_RC_OK))
			{
				viewer_disable_rdpgfx_locked(viewer);
				LeaveCriticalSection(&viewer->gfx.lock);
				WLog_WARN(TAG,
		...
		if (peer->IsWriteBlocked && peer->IsWriteBlocked(peer) && peer->DrainOutputBuffer)
		{
	...
	if (g_viewer_server)
	{
		...
		viewer_server_clear_input_owner_locked(g_viewer_server, viewer->id,
...
static BOOL
peer_post_connect(freerdp_peer* peer)
{
	...
	if (peer->context && peer->context->settings)
		gfx_enabled =
		    freerdp_settings_get_bool(peer->context->settings, FreeRDP_SupportGraphicsPipeline);

	WLog_INFO(TAG, "Viewer %u peer_post_connect: GraphicsPipeline=%d", viewer->id, gfx_enabled);
	 ...
	 * initialization. */
	if (g_viewer_server && g_viewer_server->backend && g_viewer_server->backend->context &&
	    g_viewer_server->backend->context->settings)
	{
		BOOL backend_gfx = freerdp_settings_get_bool(g_viewer_server->backend->context->settings,
	...
	EnterCriticalSection(&viewer->gfx.lock);
	viewer_graphics_context_reset(&viewer->gfx, g_viewer_server ? g_viewer_server->backend : NULL);
	...
	{
		LeaveCriticalSection(&viewer->gfx.lock);
		WLog_ERR(TAG, "Viewer %u WTSOpenServerA failed in post_connect", viewer->id);
		return FALSE;
	}

	if (!gfx_enabled)
	{
		WLog_INFO(TAG, "Viewer %u RDPEGFX disabled; using classic SurfaceBits path only",
		          viewer->id);
...
static BOOL
peer_activate(freerdp_peer* peer)
{
	Viewer* viewer = find_viewer_by_peer(peer);
	UINT64 now = platform_get_timestamp_ms();
	ViewerGfxNegotiationOutcome negotiation_outcome = VIEWER_GFX_NEGOTIATION_PENDING;
	...
	if (viewer_gfx_negotiation_is_rdpgfx_ready(&viewer->gfx))
		viewer_gfx_begin_join_locked(viewer, now, "peer activated for RDPEGFX late join");
	else if (viewer_gfx_activation_waits_for_rdpgfx_caps(&viewer->gfx))
		viewer_gfx_begin_join_locked(viewer, now,
		                             "peer activated waiting for RDPEGFX caps confirmation");
	else
	{
		viewer->gfx.join_start_ts = now;
		viewer_gfx_finish_late_join_locked(viewer, "peer activated on classic path");
	...
	if (negotiation_outcome == VIEWER_GFX_NEGOTIATION_RDPEGFX_READY)
		WLog_INFO(TAG, "Viewer %u RDPEGFX activated; evaluating frame-ring late join", viewer->id);

	if (g_viewer_server && g_viewer_server->backend)
	{
		if (negotiation_outcome == VIEWER_GFX_NEGOTIATION_RDPEGFX_READY)
		{
			WLog_INFO(TAG, "Viewer %u RDPEGFX activated; awaiting handshake-gated late join",
			          viewer->id);
		}
		else if (backend_full_refresh_in_flight(g_viewer_server->backend))
		{
			WLog_INFO(TAG, "Viewer %u activated, joining in-flight full refresh", viewer->id);
		}
		else
		{
...
static BOOL
peer_context_new(freerdp_peer* peer, rdpContext* context)
{
	ViewerServer* server = g_viewer_server;
	Viewer* viewer = NULL;
...
static void
peer_context_free(freerdp_peer* peer, rdpContext* context)
{
	ViewerServer* server = g_viewer_server;
	(void)context;
	...
	for (int i = 0; i < MAX_VIEWERS; i++)
	{
		if (server->viewers[i].peer == peer)
		{
			viewer_server_clear_input_owner_locked(server, server->viewers[i].id,
			                                       "disconnected, input ownership cleared");
 ...
 */
static BOOL
peer_reached_state(freerdp_peer* peer, CONNECTION_STATE state)
{
	...
	WLog_INFO(
	    TAG,
	    "peer_reached_state:   Before: DesktopWidth=%" PRIu32 ", DesktopHeight=%" PRIu32
	    ", MonitorCount=%" PRIu32 ", SupportMonitorLayoutPdu=%s, SupportDynamicTimeZone=%s",
	    freerdp_settings_get_uint32(settings, FreeRDP_DesktopWidth),
	    freerdp_settings_get_uint32(settings, FreeRDP_DesktopHeight),
	    freerdp_settings_get_uint32(settings, FreeRDP_MonitorCount),
	    freerdp_settings_get_bool(settings, FreeRDP_SupportMonitorLayoutPdu) ? "TRUE" : "FALSE",
	    freerdp_settings_get_bool(settings, FreeRDP_SupportDynamicTimeZone) ? "TRUE" : "FALSE");

	/* Restore desktop dimensions from backend layout */
	freerdp_settings_set_uint32(settings, FreeRDP_DesktopWidth, layout->total_width);
	freerdp_settings_set_uint32(settings, FreeRDP_DesktopHeight, layout->total_height);

	/* Restore monitor count and layout */
	freerdp_settings_set_uint32(settings, FreeRDP_MonitorCount, layout->monitor_count);
	...
	for (i = 0; i < layout->monitor_count; i++)
	{
		...
		mon.is_primary = (layout->monitors[i].flags & MONITOR_PRIMARY) ? TRUE : FALSE;
		mon.orig_screen = i;
		...
		freerdp_settings_set_pointer_array(settings, FreeRDP_MonitorDefArray, i, &mon);

		WLog_INFO(TAG,
		          "peer_reached_state:   monitor[%" PRIu32 "]: x=%" PRId32 ", y=%" PRId32
		          ", width=%" PRId32 ", height=%" PRId32 ", is_primary=%s",
		          i, mon.x, mon.y, mon.width, mon.height, mon.is_primary ? "TRUE" : "FALSE");
	}
	...
	WLog_INFO(TAG,
	          "peer_reached_state:   After: DesktopWidth=%" PRIu32 ", DesktopHeight=%" PRIu32
	          ", MonitorCount=%" PRIu32
...
static BOOL
peer_accepted(freerdp_listener* listener, freerdp_peer* peer)
{
	ViewerServer* server = g_viewer_server;
	Viewer* viewer = NULL;
	rdpSettings* settings = NULL;
	...
	if (settings)
	{
		...
		viewer_get_backend_layout(server->backend, &desktop_width, &desktop_height, NULL);
		...
		WLog_INFO(TAG, "peer_accepted: Setting desktop_size=%ux%u for viewer", desktop_width,
		          desktop_height);
		...
		WLog_INFO(
		    TAG, "peer_accepted: SupportMonitorLayoutPdu=TRUE, desktop_width=%" PRIu32 " > 1920=%s",
		...
		if (desktop_width > 1920)
		{
			if (server && server->backend)
			{
				...
				for (mi = 0; mi < layout->monitor_count; mi++)
				{
					...
					mon.is_primary = (layout->monitors[mi].flags & MONITOR_PRIMARY) ? TRUE : FALSE;
					mon.orig_screen = mi;
					mon.attributes.physicalWidth = mon.width;
					mon.attributes.physicalHeight = mon.height;
					mon.attributes.orientation = ORIENTATION_LANDSCAPE;
					...
					freerdp_settings_set_pointer_array(settings, FreeRDP_MonitorDefArray, mi, &mon);
				}

				{
					...
					for (k = 0; k < monCountAfter; k++)
					{
						...
						if (m)
							WLog_INFO(TAG,
							          "peer_accepted:   viewer settings[%" PRIu32 "]: x=%" PRId32
							          ", y=%" PRId32 ", width=%" PRId32 ", height=%" PRId32
							          ", is_primary=%s",
...

		{
			const char* cert_file = (server->cert_path && server->cert_path[0])
			                            ...
			                            : platform_cert_path();
			const char* key_file =
			    (server->key_path && server->key_path[0]) ? server->key_path : platform_key_path();
...
ViewerServer*
viewer_server_init(const char* bind_address, UINT16 port, BackendClient* backend,
                   const char* cert_path, const char* key_path)
{
	...
	server->bind_address = bind_address ? strdup(bind_address) : strdup("0.0.0.0");
...
BOOL
viewer_server_start(ViewerServer* server)
{
	if (!server || !server->listener)
		return FALSE;

	if (!server->listener->Open(server->listener, server->bind_address, server->port))
		return FALSE;
...
void
viewer_server_stop(ViewerServer* server)
{
...
void
viewer_server_free(ViewerServer* server)
{
	if (!server)
		return;
...
void
viewer_server_notify_backend_layout_change(BackendClient* backend, UINT32 width, UINT32 height,
                                           UINT32 generation)
{
	ViewerServer* server = g_viewer_server;
	...
	for (int i = 0; i < MAX_VIEWERS; i++)
	{
		...
		if (viewer->peer->context && viewer->peer->context->settings)
		{
			...
			if (width > 1920 && server && server->backend)
			{
				...
				freerdp_settings_set_uint32(vs, FreeRDP_MonitorCount, layout->monitor_count);
				...
				for (mi = 0; mi < layout->monitor_count; mi++)
				{
					...
					WLog_INFO(TAG,
					          "viewer_server_notify_backend_layout_change: viewer[%d]: "
					          "monitor[%" PRIu32 "]: x=%" PRId32 ", y=%" PRId32 ", width=%" PRId32
					          ", height=%" PRId32 ", is_primary=%s",
					          i, mi, mon.x, mon.y, mon.width, mon.height,
					          mon.is_primary ? "TRUE" : "FALSE");

					freerdp_settings_set_pointer_array(vs, FreeRDP_MonitorDefArray, mi, &mon);
				}
			}
		}
	}
...
BOOL
viewer_server_publish_surface_bits(BackendClient* backend, const SURFACE_BITS_COMMAND* cmd)
{
	ViewerServer* server = g_viewer_server;
	...
	if ((classic_target_count == 0) || (gated_full_refresh_count > 0) || (throttled_count > 0) ||
	    (enqueue_failed_count > 0) || (enqueued_count == 0))
	{
		WLog_INFO(TAG,
		          "Classic SurfaceBits publish summary rect=(%u,%u)-(%u,%u) "
		          "payload=%" PRIu32 " codecId=%" PRIu16 " classicTargets=%" PRIu32
		          " enqueued=%" PRIu32 " gatedFullRefresh=%" PRIu32 " firstGatedViewer=%" PRIu32
		          " throttled=%" PRIu32 " firstThrottledViewer=%" PRIu32 " enqueueFailed=%" PRIu32
		          " refreshInFlight=%d publishUs=%" PRIu64,
		          cmd->destLeft, cmd->destTop, cmd->destRight, cmd->destBottom,
		          cmd->bmp.bitmapDataLength, cmd->bmp.codecID, classic_target_count, enqueued_count,
		          gated_full_refresh_count, first_gated_viewer_id, throttled_count,
		          first_throttled_viewer_id, enqueue_failed_count, refresh_in_flight, publish_us);
	}
...
BOOL
viewer_server_publish_bitmap_update(BackendClient* backend, const BITMAP_UPDATE* bitmap)
{
	ViewerServer* server = g_viewer_server;
	Viewer* targets[MAX_VIEWERS] = { 0 };
	size_t target_count = 0;
	...
	{
		WLog_INFO(TAG,
		          "Classic BitmapUpdate publish summary batch=%" PRIu64 " rectangles=%" PRIu32
		          " payload=%" PRIu64 " skipCompression=%d firstRectPayload=%" PRIu32
		          " classicTargets=%" PRIu32 " enqueued=%" PRIu32 " gatedFullRefresh=%" PRIu32
		          " firstGatedViewer=%" PRIu32 " throttled=%" PRIu32
		          " firstThrottledViewer=%" PRIu32 " enqueueFailed=%" PRIu32
		          " refreshInFlight=%d publishUs=%" PRIu64,
		          backend->bitmap_update_batches_total + 1ULL, bitmap->number, total_payload_bytes,
		          bitmap->skipCompression, first_rect_payload, classic_target_count, enqueued_count,
		          gated_full_refresh_count, first_gated_viewer_id, throttled_count,
		          first_throttled_viewer_id, enqueue_failed_count, refresh_in_flight, publish_us);
	}
...
BOOL
viewer_server_publish_frame_marker(BackendClient* backend, const SURFACE_FRAME_MARKER* marker)
{
	ViewerServer* server = g_viewer_server;
	Viewer* targets[MAX_VIEWERS] = { 0 };
	size_t target_count = 0;
	...
	for (size_t i = 0; i < target_count; i++)
	{
		...
		rdpSettings* settings =
		    viewer->peer ? viewer->peer->context ? viewer->peer->context->settings : NULL : NULL;
	...
	if (marker->frameAction == SURFACECMD_FRAMEACTION_END)
	{
		for (size_t i = 0; i < target_count; i++)
		{
			...
			if (viewer->gfx.join_strategy == VIEWER_JOIN_STRATEGY_CLASSIC_FALLBACK)
				viewer_gfx_finish_late_join_locked(viewer,
				                                   "classic fallback full refresh completed");
			LeaveCriticalSection(&viewer->gfx.lock);
		}
		...
		WLog_INFO(TAG, "Full refresh completed for %u viewers (frame id=%u)", completed_viewers,
		          marker->frameId);
	}
...
BOOL
viewer_server_publish_gfx_reset_graphics(BackendClient* backend,
                                         const RDPGFX_RESET_GRAPHICS_PDU* reset_graphics)
{
	...
	WLog_INFO(TAG,
	          "ResetGraphics width=%" PRIu32 " height=%" PRIu32 " monitors=%" PRIu32
	          " clearing frame ring and capture state",
	          reset_graphics->width, reset_graphics->height, reset_graphics->monitorCount);

	EnterCriticalSection(&server->gfx.lock);
	viewer_gfx_publisher_state_reset_locked(&server->gfx);
	if (!viewer_gfx_reset_graphics_pdu_copy(&server->gfx.latest_reset_graphics, reset_graphics))
	{
...
BOOL
viewer_server_publish_gfx_create_surface(BackendClient* backend,
                                         const RDPGFX_CREATE_SURFACE_PDU* create_surface)
{
	...
	event = viewer_gfx_event_new_simple(VIEWER_GFX_EVENT_CREATE_SURFACE, create_surface,
	                                    sizeof(*create_surface));
	if (!event)
		return FALSE;

	WLog_INFO(
	    TAG, "CreateSurface surfaceId=%" PRIu16 " size=%" PRIu16 "x%" PRIu16 " format=0x%04" PRIX16,
	    create_surface->surfaceId, create_surface->width, create_surface->height,
	    create_surface->pixelFormat);

	EnterCriticalSection(&server->gfx.lock);
	surface = viewer_gfx_upsert_surface_locked(&server->gfx, create_surface->surfaceId);
	if (surface)
	{
		surface->create_surface = *create_surface;
		surface->mapped = FALSE;
		memset(&surface->map_surface_to_output, 0, sizeof(surface->map_surface_to_output));
	}
	LeaveCriticalSection(&server->gfx.lock);
...
BOOL
viewer_server_publish_gfx_delete_surface(BackendClient* backend,
                                         const RDPGFX_DELETE_SURFACE_PDU* delete_surface)
{
	...
	event = viewer_gfx_event_new_simple(VIEWER_GFX_EVENT_DELETE_SURFACE, delete_surface,
	                                    sizeof(*delete_surface));
	if (!event)
		return FALSE;
...
BOOL
viewer_server_publish_gfx_map_surface_to_output(
    BackendClient* backend, const RDPGFX_MAP_SURFACE_TO_OUTPUT_PDU* map_surface_to_output)
{
	...
	event = viewer_gfx_event_new_simple(VIEWER_GFX_EVENT_MAP_SURFACE_TO_OUTPUT,
	                                    map_surface_to_output, sizeof(*map_surface_to_output));
	if (!event)
		return FALSE;

	WLog_INFO(TAG, "MapSurfaceToOutput surfaceId=%" PRIu16 " origin=(%" PRIu32 ",%" PRIu32 ")",
	          map_surface_to_output->surfaceId, map_surface_to_output->outputOriginX,
	          map_surface_to_output->outputOriginY);

	EnterCriticalSection(&server->gfx.lock);
	surface = viewer_gfx_upsert_surface_locked(&server->gfx, map_surface_to_output->surfaceId);
	if (surface)
	{
...
BOOL
viewer_server_publish_gfx_start_frame(BackendClient* backend,
                                      const RDPGFX_START_FRAME_PDU* start_frame)
{
	ViewerServer* server = g_viewer_server;
	ViewerGfxEvent* event = NULL;
	...
	if (!viewer_gfx_frame_buffer_begin_frame_locked(&server->gfx.frame_buffer, start_frame, event))
	{
		WLog_ERR(TAG,
		         "Frame %" PRIu32 " capture failed; live forwarding continues but "
		         "replay buffer unavailable",
...
BOOL
viewer_server_publish_gfx_surface_command(BackendClient* backend, const RDPGFX_SURFACE_COMMAND* cmd)
{
	ViewerServer* server = g_viewer_server;
	ViewerGfxEvent* event = NULL;
	UINT32 frame_id = 0;
	...
	WLog_INFO(TAG,
	          "SurfaceCommand frameId=%" PRIu32 " surfaceId=%" PRIu16 " codecId=%" PRIu16
	          " length=%" PRIu32,
	          frame_id, cmd->surfaceId, cmd->codecId, cmd->length);
	if (server->gfx.in_frame && server->gfx.frame_buffer.capture_frame)
	{
		(void)viewer_gfx_frame_buffer_append_surface_command_locked(&server->gfx.frame_buffer, cmd,
		                                                            event);
	}
	else
	{
...
BOOL
viewer_server_publish_gfx_end_frame(BackendClient* backend, const RDPGFX_END_FRAME_PDU* end_frame)
{
	ViewerServer* server = g_viewer_server;
	ViewerGfxEvent* event = NULL;
	ViewerGfxCompleteFrame* latest = NULL;
	...
	event = viewer_gfx_event_new_simple(VIEWER_GFX_EVENT_END_FRAME, end_frame, sizeof(*end_frame));
	if (!event)
		return FALSE;

	EnterCriticalSection(&server->gfx.lock);
	WLog_INFO(TAG, "EndFrame received frameId=%" PRIu32, end_frame->frameId);
	if (viewer_gfx_frame_buffer_end_frame_locked(&server->gfx.frame_buffer, end_frame, event))
	{
		...
		if (latest)
		{
			WLog_INFO(TAG,
			          ...
			          latest->frame_id, latest->event_count, latest->surface_command_count,
			          latest->total_payload_bytes);
		}
	}
	if (server->gfx.in_frame && (server->gfx.current_frame_id == end_frame->frameId))
	{
...
viewer_server_publish_gfx_delete_encoding_context(
    BackendClient* backend, const RDPGFX_DELETE_ENCODING_CONTEXT_PDU* delete_encoding_context)
{
	ViewerServer* server = g_viewer_server;
	ViewerGfxEvent* event = NULL;
	...
	event = viewer_gfx_event_new_simple(VIEWER_GFX_EVENT_DELETE_ENCODING_CONTEXT,
	                                    delete_encoding_context, sizeof(*delete_encoding_context));
	if (!event)
		return FALSE;

	WLog_INFO(TAG, "DeleteEncodingContext surfaceId=%" PRIu16 " codecContextId=%" PRIu32,
	          delete_encoding_context->surfaceId, delete_encoding_context->codecContextId);
