...
static void
pointer_shape_entry_reset(PointerShapeEntry* entry)
{
	if (!entry)
		return;
...
static BOOL
pointer_shape_copy_mask(BYTE** destination, const BYTE* source, UINT32 length)
{
	BYTE* buffer = NULL;
...
static PointerShapeEntry*
pointer_shape_cache_add(PointerShapeCache* cache, UINT16 cacheIndex, UINT16 width, UINT16 height,
                        ...
                        UINT32 xorMaskLength, const BYTE* andMaskData, UINT32 andMaskLength)
{
	...
	if (!pointer_shape_copy_mask(&entry->xorMaskData, xorMaskData, xorMaskLength) ||
	    !pointer_shape_copy_mask(&entry->andMaskData, andMaskData, andMaskLength))
	{
...
PointerShapeCache*
pointer_shape_cache_new(void)
{
	return (PointerShapeCache*)calloc(1, sizeof(PointerShapeCache));
}

void
pointer_shape_cache_free(PointerShapeCache* cache)
{
	UINT32 index = 0;
                                   ...
                                   const POINTER_COLOR_UPDATE* pointer_color)
{
	...
	return pointer_shape_cache_add(
	    cache, pointer_color->cacheIndex, pointer_color->width, pointer_color->height,
	    pointer_color->hotSpotX, pointer_color->hotSpotY, 0, pointer_color->xorMaskData,
	    pointer_color->lengthXorMask, pointer_color->andMaskData, pointer_color->lengthAndMask);
}

PointerShapeEntry*
pointer_shape_cache_add_from_new(PointerShapeCache* cache, const POINTER_NEW_UPDATE* pointer_new)
{
	...
	return pointer_shape_cache_add(cache, pointer_color->cacheIndex, pointer_color->width,
	                               pointer_color->height, pointer_color->hotSpotX,
	                               pointer_color->hotSpotY, (UINT16)pointer_new->xorBpp,
	                               pointer_color->xorMaskData, pointer_color->lengthXorMask,
	                               pointer_color->andMaskData, pointer_color->lengthAndMask);
}

PointerShapeEntry*
pointer_shape_cache_get_by_index(PointerShapeCache* cache, UINT16 cacheIndex)
{
	UINT32 index = 0;
