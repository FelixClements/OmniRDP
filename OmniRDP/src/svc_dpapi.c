 ...
 */
static int
b64_encode(const unsigned char* in, size_t in_len, char* out, size_t out_size)
{
 ...
 */
static int
b64_decode(const char* in, size_t in_len, unsigned char* out, size_t out_size)
{
...
int
svc_dpapi_is_encrypted(const char* password)
{
	if (!password)
		return 0;
...
int
svc_dpapi_decrypt(const char* encrypted_password, char* plaintext_out, size_t plaintext_out_size)
{
	...
	BOOL ok = CryptUnprotectData(&encrypted_blob, NULL, /* ppszDataDescr – not needed */
...
int
svc_dpapi_encrypt(const char* plaintext, char* encrypted_out, size_t encrypted_out_size)
{
	...
	int b64_len =
	    b64_encode(encrypted_blob.pbData, (size_t)encrypted_blob.cbData, b64_buf, b64_buf_size);
...
int
svc_dpapi_encrypt_in_file(const char* config_path, const char* instance_name, const char* key)
{
					...
					if (trimmed_key_len > 0)
					{
						...
						if (keys_match)
						{
							...
							if (strncmp(value, DPAPI_PREFIX, strlen(DPAPI_PREFIX)) != 0)
							{
								/* Encrypt the value */
								char encrypted[4096];
								if (svc_dpapi_encrypt(value, encrypted, sizeof(encrypted)) == 0)
								{
									/* Write modified line */
									fprintf(tmp, "%s = %s\n", key, encrypted);
									target_modified = 1;
