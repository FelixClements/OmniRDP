 ...
 */
static int b64_encode(const unsigned char *in, size_t in_len,
...
int svc_dpapi_encrypt(const char *plaintext,
    ...
    int b64_len = b64_encode(encrypted_blob.pbData,
                             (size_t)encrypted_blob.cbData,
                ...
                if (key_len > 0) {
                    ...
                    if (trimmed_key_len > 0) {
                        ...
                        if (keys_match) {
                            ...
                            while (*val_start &&
                                   isspace((unsigned char)*val_start))
                                val_start++;
                            ...
                            if (strncmp(value, DPAPI_PREFIX,
                                        strlen(DPAPI_PREFIX)) != 0) {
                                /* Encrypt the value */
                                char encrypted[4096];
