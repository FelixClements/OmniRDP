...
write_all(HANDLE hPipe, const void* buf, DWORD total_bytes)
{
	...
	{
		if (!WriteFile(hPipe, (const BYTE*)buf + offset, total_bytes - offset, &written, NULL))
		{
			return FALSE;
		}
		offset += written;
...
read_all(HANDLE hPipe, void* buf, DWORD total_bytes)
{
	...
	while (offset < total_bytes)
	{
		if (!ReadFile(hPipe, (BYTE*)buf + offset, total_bytes - offset, &read, NULL))
		{
			return FALSE;
		}
		if (read == 0)
...
pipe_frame_recv(HANDLE hPipe, char** payload_out, DWORD* payload_len_out)
{
	...
	*payload_len_out = 0;
	...
	if (payload_len > PIPE_MAX_PAYLOAD_SIZE)
	{
		LOG_D("pipe_protocol", "frame_recv: error, payloadLen=%lu exceeds max %lu", payload_len,
		      (unsigned long)PIPE_MAX_PAYLOAD_SIZE);
		SetLastError(ERROR_BAD_LENGTH);
		return FALSE;
	}
	...
	if (payload_len > 0)
	{
		...
		if (buf == NULL)
		{
			LOG_D("pipe_protocol", "frame_recv: error, out of memory (payloadLen=%lu)",
			      payload_len);
