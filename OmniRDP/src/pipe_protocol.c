 ...
 * GetLastError()).
 * ────────────────────────────────────────────────────────────────── */
static BOOL write_all(HANDLE hPipe, const void *buf, DWORD total_bytes) {
  DWORD offset = 0;
  DWORD written = 0;

  while (offset < total_bytes) {
    if (!WriteFile(hPipe, (const BYTE *)buf + offset, total_bytes - offset,
                   &written, NULL)) {
      return FALSE;
    }
    offset += written;
 ...
 * ────────────────────────────────────────────────────────────────── */
static BOOL read_all(HANDLE hPipe, void *buf, DWORD total_bytes) {
  DWORD offset = 0;
  DWORD read = 0;

  while (offset < total_bytes) {
    if (!ReadFile(hPipe, (BYTE *)buf + offset, total_bytes - offset, &read,
                  NULL)) {
      return FALSE;
    }
    if (read == 0) {
...
BOOL pipe_frame_send(HANDLE hPipe, const char *json_payload,
                     DWORD payload_len) {
  DWORD len_le;
...
BOOL pipe_frame_recv(HANDLE hPipe, char **payload_out, DWORD *payload_len_out) {
  ...
  *payload_len_out = 0;
  ...
  if (!read_all(hPipe, &payload_len, PIPE_HEADER_SIZE)) {
    LOG_D("pipe_protocol", "frame_recv: error reading header, lastError=%lu",
          GetLastError());
    return FALSE;
  }

  LOG_D("pipe_protocol", "frame_recv: read length prefix, payloadLen=%lu",
        payload_len);

  /* ── Validate payload length ───────────────────────────────── */
  if (payload_len > PIPE_MAX_PAYLOAD_SIZE) {
    LOG_D("pipe_protocol", "frame_recv: error, payloadLen=%lu exceeds max %lu",
          payload_len, (unsigned long)PIPE_MAX_PAYLOAD_SIZE);
    SetLastError(ERROR_BAD_LENGTH);
    return FALSE;
  }
  ...
  if (payload_len > 0) {
    ...
    if (buf == NULL) {
      LOG_D("pipe_protocol",
            "frame_recv: error, out of memory (payloadLen=%lu)", payload_len);
    ...
    if (!read_all(hPipe, buf, payload_len)) {
      LOG_D("pipe_protocol", "frame_recv: error reading payload, lastError=%lu",
            GetLastError());
      HeapFree(GetProcessHeap(), 0, buf);
      return FALSE;
    }

    LOG_D("pipe_protocol", "frame_recv: read payload, bytesRead=%lu",
          payload_len);
    ...
    if (buf == NULL) {
      LOG_D("pipe_protocol",
            "frame_recv: error, out of memory for empty payload");
      SetLastError(ERROR_OUTOFMEMORY);
      return FALSE;
    }
    buf[0] = '\0';
