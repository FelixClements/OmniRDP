...
heartbeat_thread(LPVOID param)
{
	...
	{
		LOG_W("heartbeat", "Failed to connect to heartbeat pipe for '%s' (error %lu)", instanceName,
		      GetLastError());
		return 1;
	}
	...
	while (g_running)
	{
		...
		char msg[64];
		int len =
		    snprintf(msg, sizeof(msg), "heartbeat:%llu\n", (unsigned long long)GetTickCount64());
		DWORD written;
...
static void
instance_shutdown_handler(void)
{
	g_running = 0;
	if (g_server)
...
static DWORD WINAPI
instance_server_thread(LPVOID arg)
{
 ...
 */
static int
read_password_from_pipe(SIZE_T handle_value, char* password_buf, size_t password_buf_size)
{
...
static int
parse_instance_args(int argc, char* argv[], InstanceRunnerArgs* args)
{
 ...
 */
int
instance_runner_main(int argc, char* argv[])
{
#ifdef _WIN32
	WSADATA wsaData;
	...
	LOG_I("instance_runner", "Backend target: %s:%u, Viewer: %s:%u, Monitors: %u",
	      inst->backend_hostname, inst->backend_port, inst->viewer_bind_address, inst->viewer_port,
	      inst->display_monitor_count);

	/* Initialize logging */
	{
		const char* log_dir = config->service.log_dir[0] != '\0' ? config->service.log_dir
		                                                         : "C:\\ProgramData\\OmniRDP\\logs";
		char instance_log_dir[512];
		snprintf(instance_log_dir, sizeof(instance_log_dir), "%s\\%s", log_dir, args.instance_name);
		svc_log_init(instance_log_dir, SVC_LOG_DEBUG, 10, 5);
		LOG_I("instance_runner", "Instance '%s' starting (config=%s)", args.instance_name,
		      args.config_path);
	...
	{
		LOG_E("instance_runner", "Failed to connect to backend %s:%u", inst->backend_hostname,
		      inst->backend_port);
		backend_free(client);
		svc_config_free(config);
		return 1;
	...
	LOG_I("instance_runner", "Connected to backend %s:%u successfully", inst->backend_hostname,
	      inst->backend_port);

	/* Initialize viewer server */
	ViewerServer* server = viewer_server_init(inst->viewer_bind_address, inst->viewer_port, client,
	...
	LOG_I("instance_runner", "Viewer server started on %s:%u", inst->viewer_bind_address,
	      inst->viewer_port);

	/* Start heartbeat thread */
	HANDLE hHeartbeat =
	    CreateThread(NULL, 0, heartbeat_thread, (LPVOID)args.instance_name, 0, NULL);
	if (!hHeartbeat)
	{
