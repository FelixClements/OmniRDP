...
BOOL
viewer_gfx_select_compatible_caps(const RDPGFX_CAPSET* canonical_caps, BOOL canonical_caps_valid,
                                  const RDPGFX_CAPSET* advertised_caps,
                                  UINT16 advertised_caps_count, RDPGFX_CAPSET* selected_caps)
{
	UINT16 i = 0;
...
viewer_gfx_codec_replay_policy(UINT16 codec_id, const RDPGFX_CAPSET* confirmed_caps)
{
	switch (codec_id)
	{
...
BOOL
viewer_late_join_ack_releases_live(UINT32 required_ack_frame_id, UINT32 last_ack_frame_id)
{
...
BOOL
viewer_late_join_timeout_fallback_due(ViewerJoinStrategy strategy, UINT64 late_join_start_ts,
                                      UINT64 now, UINT32 timeout_ms, BOOL waiting_for_ack)
{
...
BOOL
viewer_gfx_activation_waits_for_rdpgfx_caps(const ViewerGraphicsContext* gfx)
{
...
BOOL
viewer_gfx_pending_activation_begins_rdpgfx_join(const ViewerGraphicsContext* gfx)
{
...
BOOL
viewer_gfx_vcm_progress_should_be_pumped(BOOL viewer_activated, const ViewerGraphicsContext* gfx)
{
	return viewer_activated && gfx && gfx->post_connect_complete && gfx->vcm && gfx->drdynvc_joined;
}

BOOL
viewer_gfx_drdynvc_initialization_should_run(const ViewerGraphicsContext* gfx)
{
...
BOOL
viewer_gfx_rdpgfx_open_should_run(const ViewerGraphicsContext* gfx)
{
	return gfx && gfx->vcm && gfx->post_connect_complete &&
...
BOOL
viewer_gfx_negotiation_is_pending(const ViewerGraphicsContext* gfx)
{
	return gfx && (gfx->negotiation_outcome == VIEWER_GFX_NEGOTIATION_PENDING);
}

BOOL
viewer_gfx_negotiation_is_rdpgfx_ready(const ViewerGraphicsContext* gfx)
{
	return gfx && (gfx->negotiation_outcome == VIEWER_GFX_NEGOTIATION_RDPEGFX_READY);
}

BOOL
viewer_gfx_negotiation_is_classic_fallback(const ViewerGraphicsContext* gfx)
{
	return gfx && (gfx->negotiation_outcome == VIEWER_GFX_NEGOTIATION_CLASSIC_FALLBACK);
}

BOOL
viewer_gfx_pending_activation_timeout_due(const ViewerGraphicsContext* gfx, UINT64 now,
                                          UINT32 timeout_ms)
{
...
BOOL
viewer_input_try_acquire(ViewerInputOwnershipState* state, UINT32 viewer_id, BOOL viewer_connected,
                         BOOL viewer_activated, BOOL owner_alive, UINT64 now, UINT64 timeout_ms)
{
...
UINT32
viewer_slot_index_to_id(UINT32 slot_index)
{
	return slot_index + 1U;
}

BOOL
viewer_is_slow(UINT32 consecutive_lag_intervals, BOOL write_blocked)
{
	return write_blocked || (consecutive_lag_intervals >= VIEWER_SEVERE_LAG_INTERVALS);
}

BOOL
viewer_lag_signal_active(const Viewer* viewer, BOOL write_blocked)
{
	return viewer &&
	       (write_blocked || (viewer->consecutive_lag_intervals >= VIEWER_SEVERE_LAG_INTERVALS));
}

BOOL
viewer_disconnect_due(const Viewer* viewer, UINT32 disconnect_ms, UINT64 now)
{
...
BOOL
viewer_monitor_from_size(UINT32 width, UINT32 height, MONITOR_DEF* monitor)
{
...
void
monitor_layout_init(MonitorLayout* layout, UINT32 monitor_count)
{
	UINT32 i = 0;
	...
	fprintf(stderr,
	        "[viewer.internal] monitor_layout_init: monitor_count=%" PRIu32 ", total_width=%" PRIu32
	        ", total_height=%" PRIu32 "\n",
	...
	for (i = 0; i < monitor_count; i++)
	{
		...
		fprintf(stderr,
		        "[viewer.internal] monitor_layout_init: monitor[%" PRIu32 "]: left=%" PRId32
		        ", top=%" PRId32 ", right=%" PRId32 ", bottom=%" PRId32 ", flags=0x%08" PRIx32
		        "%s\n",
		        i, layout->monitors[i].left, layout->monitors[i].top, layout->monitors[i].right,
		        layout->monitors[i].bottom, layout->monitors[i].flags,
