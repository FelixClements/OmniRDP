...
static void
svc_config_default_service(SvcServiceConfig* cfg)
{
	memset(cfg, 0, sizeof(*cfg));
	strncpy(cfg->log_level, "info", sizeof(cfg->log_level) - 1);
	cfg->log_level[sizeof(cfg->log_level) - 1] = '\0';
	strncpy(cfg->log_dir, "C:\\ProgramData\\OmniRDP\\logs", sizeof(cfg->log_dir) - 1);
...
static void
svc_config_default_instance(InstanceConfig* cfg)
{
	memset(cfg, 0, sizeof(*cfg));
	cfg->enabled = 1;
	...
	strncpy(cfg->viewer_bind_address, "127.0.0.1", sizeof(cfg->viewer_bind_address) - 1);
	...
	strncpy(cfg->security_tls_min_version, "1.2", sizeof(cfg->security_tls_min_version) - 1);
	cfg->security_tls_min_version[sizeof(cfg->security_tls_min_version) - 1] = '\0';
...
static int
strcpy_safe(char* dest, size_t dest_size, const char* src)
{
	...
	{
		fprintf(stderr, "Warning: config value truncated: '%s' -> '%s' (max %zu chars)\n", src,
		        dest, dest_size - 1);
		return -1;
	}
	return 0;
...
static int
parse_one_instance(const IniFile* ini, const char* name, InstanceConfig* inst)
{
	char section[256];
	...
	inst->backend_port = (uint16_t)ini_get_uint(ini, section, "backend.port", inst->backend_port);
	...
	inst->backend_connect_timeout_ms =
	    ini_get_uint(ini, section, "backend.connect_timeout_ms", inst->backend_connect_timeout_ms);

	/* Reconnect policy */
	inst->reconnect_enabled =
	    ini_get_bool(ini, section, "reconnect.enabled", inst->reconnect_enabled);
	inst->reconnect_max_attempts =
	    ini_get_uint(ini, section, "reconnect.max_attempts", inst->reconnect_max_attempts);
	inst->reconnect_initial_delay_ms =
	    ini_get_uint(ini, section, "reconnect.initial_delay_ms", inst->reconnect_initial_delay_ms);
	inst->reconnect_max_delay_ms =
	    ini_get_uint(ini, section, "reconnect.max_delay_ms", inst->reconnect_max_delay_ms);
	{
		const char* bm = ini_get(ini, section, "reconnect.backoff_multiplier", NULL);
		if (bm)
			inst->reconnect_backoff_multiplier = strtod(bm, NULL);
	}

	/* Viewer listener */
	strcpy_safe(inst->viewer_bind_address, sizeof(inst->viewer_bind_address),
	            ini_get(ini, section, "viewer.bind_address", inst->viewer_bind_address));
	inst->viewer_port = (uint16_t)ini_get_uint(ini, section, "viewer.port", inst->viewer_port);
	inst->viewer_max_viewers =
	    ini_get_uint(ini, section, "viewer.max_viewers", inst->viewer_max_viewers);
	...
	inst->viewer_slow_disconnect_enabled = ini_get_bool(
	    ini, section, "viewer.slow_disconnect_enabled", inst->viewer_slow_disconnect_enabled);
	inst->viewer_slow_lag_interval_ms = ini_get_uint(ini, section, "viewer.slow_lag_interval_ms",
	                                                 inst->viewer_slow_lag_interval_ms);
	inst->viewer_slow_disconnect_after_ms = ini_get_uint(
	    ini, section, "viewer.slow_disconnect_after_ms", inst->viewer_slow_disconnect_after_ms);
	inst->viewer_late_join_timeout_ms = ini_get_uint(ini, section, "viewer.late_join_timeout_ms",
	...
	/* Display */
	inst->display_monitor_count =
	    ini_get_uint(ini, section, "display.monitor_count", inst->display_monitor_count);
	inst->display_monitor_width =
	    ini_get_uint(ini, section, "display.monitor_width", inst->display_monitor_width);
	inst->display_monitor_height =
	    ini_get_uint(ini, section, "display.monitor_height", inst->display_monitor_height);
	inst->display_color_depth =
	    ini_get_uint(ini, section, "display.color_depth", inst->display_color_depth);

	/* Codec */
	inst->codec_nscodec = ini_get_bool(ini, section, "codec.nscodec", inst->codec_nscodec);
	inst->codec_remote_fx = ini_get_bool(ini, section, "codec.remote_fx", inst->codec_remote_fx);
	inst->codec_graphics_pipeline =
	    ini_get_bool(ini, section, "codec.graphics_pipeline", inst->codec_graphics_pipeline);
	inst->codec_h264 = ini_get_bool(ini, section, "codec.h264", inst->codec_h264);
	inst->codec_avc444 = ini_get_bool(ini, section, "codec.avc444", inst->codec_avc444);
	inst->codec_avc444v2 = ini_get_bool(ini, section, "codec.avc444v2", inst->codec_avc444v2);
	inst->codec_frame_acknowledge =
	    ini_get_uint(ini, section, "codec.frame_acknowledge", inst->codec_frame_acknowledge);

	/* Security */
	inst->security_tls_enabled =
	    ini_get_bool(ini, section, "security.tls_enabled", inst->security_tls_enabled);
	inst->security_nla_enabled =
	    ini_get_bool(ini, section, "security.nla_enabled", inst->security_nla_enabled);
	strcpy_safe(inst->security_tls_min_version, sizeof(inst->security_tls_min_version),
	            ini_get(ini, section, "security.tls_min_version", inst->security_tls_min_version));
	inst->security_server_authentication = ini_get_bool(
	    ini, section, "security.server_authentication", inst->security_server_authentication);
	inst->security_ignore_certificate = ini_get_bool(ini, section, "security.ignore_certificate",
	                                                 inst->security_ignore_certificate);

	/* Validate required fields */
	if (inst->backend_hostname[0] == '\0')
	{
		fprintf(stderr, "Instance '%s': missing required field 'backend.hostname'\n", name);
		return -1;
	}
	if (inst->backend_username[0] == '\0')
	{
		fprintf(stderr, "Instance '%s': missing required field 'backend.username'\n", name);
		return -1;
	}
...
SvcConfig*
svc_config_create_default(void)
{
...
SvcConfig*
svc_config_load(const char* filename)
{
	if (!filename)
		return NULL;
	...
	/* ── Parse [service] section ───────────────────────────── */
	{
		...
		svc->log_max_files = ini_get_uint(ini, "service", "log_max_files", svc->log_max_files);
		strcpy_safe(svc->pipe_name, sizeof(svc->pipe_name),
		            ini_get(ini, "service", "pipe_name", svc->pipe_name));
		svc->heartbeat_timeout_sec =
		    ini_get_uint(ini, "service", "heartbeat_timeout_sec", svc->heartbeat_timeout_sec);
		svc->graceful_shutdown_sec =
		    ini_get_uint(ini, "service", "graceful_shutdown_sec", svc->graceful_shutdown_sec);
		svc->health_poll_interval_sec =
		    ini_get_uint(ini, "service", "health_poll_interval_sec", svc->health_poll_interval_sec);
		svc->instance_startup_delay_ms = ini_get_uint(ini, "service", "instance_startup_delay_ms",
...
svc_config_find_instance(const SvcConfig* config, const char* name)
{
	if (!config || !name)
		return NULL;
...
void
svc_config_free(SvcConfig* config)
{
	if (!config)
		return;
