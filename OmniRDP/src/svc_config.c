...
static void svc_config_default_service(SvcServiceConfig *cfg) {
    ...
    cfg->log_max_files           = 5;
    strncpy(cfg->pipe_name, "OmniRDP_ServicePipe",
            sizeof(cfg->pipe_name) - 1);
...
static int parse_one_instance(const IniFile *ini, const char *name,
    ...
    inst->backend_port = (uint16_t)ini_get_uint(ini, section, "backend.port",
                                                 inst->backend_port);
    ...
    inst->reconnect_enabled =
        ini_get_bool(ini, section, "reconnect.enabled",
                     inst->reconnect_enabled);
    inst->reconnect_max_attempts =
        ini_get_uint(ini, section, "reconnect.max_attempts",
                     inst->reconnect_max_attempts);
    inst->reconnect_initial_delay_ms =
        ini_get_uint(ini, section, "reconnect.initial_delay_ms",
                     inst->reconnect_initial_delay_ms);
    inst->reconnect_max_delay_ms =
        ini_get_uint(ini, section, "reconnect.max_delay_ms",
                     inst->reconnect_max_delay_ms);
    {
        const char *bm = ini_get(ini, section, "reconnect.backoff_multiplier",
                                 NULL);
        if (bm)
    ...
    strcpy_safe(inst->viewer_bind_address, sizeof(inst->viewer_bind_address),
                ini_get(ini, section, "viewer.bind_address",
                        inst->viewer_bind_address));
    inst->viewer_port = (uint16_t)ini_get_uint(ini, section, "viewer.port",
                                                inst->viewer_port);
    inst->viewer_max_viewers =
        ini_get_uint(ini, section, "viewer.max_viewers",
                     inst->viewer_max_viewers);
    ...
    inst->display_monitor_count =
        ini_get_uint(ini, section, "display.monitor_count",
                     inst->display_monitor_count);
    inst->display_monitor_width =
        ini_get_uint(ini, section, "display.monitor_width",
                     inst->display_monitor_width);
    inst->display_monitor_height =
        ini_get_uint(ini, section, "display.monitor_height",
                     inst->display_monitor_height);
    inst->display_color_depth =
        ini_get_uint(ini, section, "display.color_depth",
                     inst->display_color_depth);
    ...
    inst->codec_graphics_pipeline =
        ini_get_bool(ini, section, "codec.graphics_pipeline",
                     inst->codec_graphics_pipeline);
    inst->codec_h264 =
        ini_get_bool(ini, section, "codec.h264", inst->codec_h264);
    inst->codec_avc444 =
    ...
    inst->codec_frame_acknowledge =
        ini_get_uint(ini, section, "codec.frame_acknowledge",
                     inst->codec_frame_acknowledge);

    /* Security */
    inst->security_tls_enabled =
        ini_get_bool(ini, section, "security.tls_enabled",
                     inst->security_tls_enabled);
    inst->security_nla_enabled =
        ini_get_bool(ini, section, "security.nla_enabled",
                     inst->security_nla_enabled);
...
SvcConfig *svc_config_load(const char *filename) {
    ...
    /* ── Parse [service] section ───────────────────────────── */
    {
        ...
        svc->log_max_size_mb =
            ini_get_uint(ini, "service", "log_max_size_mb",
                         svc->log_max_size_mb);
        svc->log_max_files =
            ini_get_uint(ini, "service", "log_max_files",
                         svc->log_max_files);
        strcpy_safe(svc->pipe_name, sizeof(svc->pipe_name),
                    ini_get(ini, "service", "pipe_name", svc->pipe_name));
        svc->heartbeat_timeout_sec =
            ini_get_uint(ini, "service", "heartbeat_timeout_sec",
                         svc->heartbeat_timeout_sec);
        svc->graceful_shutdown_sec =
            ini_get_uint(ini, "service", "graceful_shutdown_sec",
                         svc->graceful_shutdown_sec);
    ...
    if (names_str && names_str[0] != '\0') {
        ...
        config->instances =
            (InstanceConfig *)calloc(count, sizeof(InstanceConfig));
        if (!config->instances) {
            free(names_copy);
            svc_config_free(config);
            return NULL;
        ...
        while (token && idx < count) {
            ...
            if (token[0] != '\0') {
                ...
                if (parse_one_instance(ini, token,
                                        &config->instances[idx]) != 0) {
                    free(names_copy);
                    svc_config_free(config);
                    return NULL;
                }
