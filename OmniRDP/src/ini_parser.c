...
static IniEntry *ini_find(const IniFile *ini, const char *section,
                          const char *key) {
...
static int ini_push(IniFile *ini, char *section, char *key, char *value) {
  ...
  if (ini->count >= ini->capacity) {
    ...
    IniEntry *new_entries =
        (IniEntry *)realloc(ini->entries, new_cap * sizeof(IniEntry));
    if (!new_entries) {
      free(section);
      free(key);
      free(value);
...
int ini_get_int(const IniFile *ini, const char *section, const char *key,
                int default_val) {
...
unsigned int ini_get_uint(const IniFile *ini, const char *section,
                          const char *key, unsigned int default_val) {
  const char *val = ini_get(ini, section, key, NULL);
  if (!val)
    return default_val;
  char *end;
...
int ini_get_bool(const IniFile *ini, const char *section, const char *key,
                 int default_val) {
  const char *val = ini_get(ini, section, key, NULL);
  if (!val)
    return default_val;
  ...
  if (_stricmp(val, "true") == 0 || strcmp(val, "1") == 0 ||
      _stricmp(val, "yes") == 0 || _stricmp(val, "on") == 0) {
    return 1;
  }
  if (_stricmp(val, "false") == 0 || strcmp(val, "0") == 0 ||
      _stricmp(val, "no") == 0 || _stricmp(val, "off") == 0) {
    return 0;
  }
  return default_val;
}
