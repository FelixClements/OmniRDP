...
load_log_content(void)
{
	...
	/* Read content */
	char* content = (char*)HeapAlloc(GetProcessHeap(), 0, (size_t)(fileSize - skip + 1));
	if (content)
	{
...
log_viewer_wndproc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
		case WM_CREATE:
...
HWND
tray_log_viewer_show(HINSTANCE hInstance, const char* logPath)
{
	if (g_hLogViewer)
	{
	...
	g_hLogViewer = CreateWindowExA(WS_EX_OVERLAPPEDWINDOW, "OmniRDPLogViewer",
...
BOOL
tray_log_viewer_is_visible(void)
{
	return g_hLogViewer != NULL;
}
