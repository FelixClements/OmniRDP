...
static void load_log_content(void) {
    if (!g_hEdit || g_logPath[0] == '\0') return;
...
static LRESULT CALLBACK log_viewer_wndproc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE:
            ...
            /* Set monospace font */
            {
                ...
                if (hFont) SendMessage(g_hEdit, WM_SETFONT, (WPARAM)hFont, TRUE);
            }
            load_log_content();
