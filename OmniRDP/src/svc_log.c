...
static const char *svc_log_level_name(SvcLogLevel level) {
    switch (level) {
        case SVC_LOG_DEBUG: return "DEBUG";
        case SVC_LOG_INFO:  return "INFO";
        case SVC_LOG_WARN:  return "WARN";
        case SVC_LOG_ERROR: return "ERROR";
        default:            return "?";
    }
}
...
int svc_log_level_from_string(const char *str, SvcLogLevel *out) {
    ...
    if (_stricmp(str, "debug") == 0)  { *out = SVC_LOG_DEBUG; return 0; }
    if (_stricmp(str, "info")  == 0)  { *out = SVC_LOG_INFO;  return 0; }
    if (_stricmp(str, "warn")  == 0)  { *out = SVC_LOG_WARN;  return 0; }
    if (_stricmp(str, "error") == 0)  { *out = SVC_LOG_ERROR; return 0; }
...
static void svc_log_rotate_internal(void) {
    ...
    _snprintf(oldpath, sizeof(oldpath), "%s\\%s.%u",
              g_log_dir, LOG_FILE_NAME, g_max_files);
    DeleteFileA(oldpath);

    /* 3. Shift the chain: (N-1) -> N, ..., 1 -> 2 */
    for (unsigned int i = g_max_files - 1; i >= 1; i--) {
        _snprintf(oldpath, sizeof(oldpath), "%s\\%s.%u",
                  g_log_dir, LOG_FILE_NAME, i);
        _snprintf(newpath, sizeof(newpath), "%s\\%s.%u",
                  g_log_dir, LOG_FILE_NAME, i + 1);
        MoveFileExA(oldpath, newpath, MOVEFILE_REPLACE_EXISTING);
    }

    /* 4. Rename current log -> .1 */
    _snprintf(newpath, sizeof(newpath), "%s\\%s.1",
              g_log_dir, LOG_FILE_NAME);
...
static void svc_log_timestamp(char *buf, size_t size) {
    ...
    SYSTEMTIME st;
    GetLocalTime(&st);
    _snprintf(buf, size, "%04u-%02u-%02u %02u:%02u:%02u",
...
int svc_log_init(const char *log_dir, SvcLogLevel log_level,
    ...
    int ret = _snprintf(g_log_path, sizeof(g_log_path),
                        "%s\\%s", log_dir, LOG_FILE_NAME);
...
void svc_log_write(SvcLogLevel level, const char *source,
    ...
    if (!g_init) {
        ...
        int n = _snprintf(debug_buf, sizeof(debug_buf),
                          "[svc_log] [%s] [%s] ",
                          svc_log_level_name(level),
                          source ? source : "?");

        if (n > 0 && (size_t)n < sizeof(debug_buf)) {
            va_list args;
            va_start(args, fmt);
            _vsnprintf(debug_buf + n, sizeof(debug_buf) - (size_t)n,
                       fmt, args);
            va_end(args);
        }
