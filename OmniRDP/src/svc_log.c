 ...
 */
static int
svc_log_mkdir_recursive(const char* path)
{
	if (!path || path[0] == '\0')
		return -1;
...
static const char*
svc_log_level_name(SvcLogLevel level)
{
	switch (level)
	{
		case SVC_LOG_DEBUG:
 ...
 */
int
svc_log_level_from_string(const char* str, SvcLogLevel* out)
{
	if (!str || !out)
		return -1;
...
svc_log_rotate_internal(void)
{
	...
	_snprintf(oldpath, sizeof(oldpath), "%s\\%s.%u", g_log_dir, LOG_FILE_NAME, g_max_files);
	DeleteFileA(oldpath);

	/* 3. Shift the chain: (N-1) -> N, ..., 1 -> 2 */
	for (unsigned int i = g_max_files - 1; i >= 1; i--)
	{
		_snprintf(oldpath, sizeof(oldpath), "%s\\%s.%u", g_log_dir, LOG_FILE_NAME, i);
		_snprintf(newpath, sizeof(newpath), "%s\\%s.%u", g_log_dir, LOG_FILE_NAME, i + 1);
...
svc_log_check_rotate(void)
{
	...
	unsigned long long max_bytes = (unsigned long long)g_max_size * 1024ULL * 1024ULL;
 ...
 */
static void
svc_log_timestamp(char* buf, size_t size)
{
	if (!buf || size == 0)
		return;
	...
	SYSTEMTIME st;
	GetLocalTime(&st);
	_snprintf(buf, size, "%04u-%02u-%02u %02u:%02u:%02u", st.wYear, st.wMonth, st.wDay, st.wHour,
...
int
svc_log_init(const char* log_dir, SvcLogLevel log_level, unsigned int max_size_mb,
             unsigned int max_files)
{
	...
	int ret = _snprintf(g_log_path, sizeof(g_log_path), "%s\\%s", log_dir, LOG_FILE_NAME);
...
void
svc_log_write(SvcLogLevel level, const char* source, const char* fmt, ...)
{
	/*
	...
	int n = _snprintf(line, sizeof(line), "[%s] [%s] [%s] ", ts, svc_log_level_name(level),
	                  source ? source : "?");
