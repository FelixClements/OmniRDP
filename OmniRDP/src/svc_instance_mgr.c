...
int inst_mgr_init(InstanceManager *mgr, SvcConfig *config,
  ...
  mgr->instances = (ManagedInstance *)calloc(config->instance_count,
                                             sizeof(ManagedInstance));
...
int inst_mgr_start(InstanceManager *mgr, const char *instanceName) {
  ...
  if (mgr->shuttingDown) {
    LOG_W("svc_inst_mgr",
          "Cannot start instance '%s': service is shutting down", instanceName);
    return -1;
  }
  ...
  /* ── Create heartbeat named pipe for this instance ─────────── */
  {
    ...
    build_heartbeat_pipe_name(instanceName, heartbeatPipeName,
                              sizeof(heartbeatPipeName));
    inst->hHeartbeatPipe = CreateNamedPipeA(
        heartbeatPipeName, PIPE_ACCESS_INBOUND | FILE_FLAG_OVERLAPPED,
...
void inst_mgr_poll(InstanceManager *mgr) {
  ...
  for (unsigned int i = 0; i < mgr->instanceCount; i++) {
    ...
    if (inst->state == INST_RUNNING && inst->hProcess != NULL) {
      ...
      ULONGLONG timeoutMs =
          (ULONGLONG)mgr->config->service.heartbeat_timeout_sec * 1000ULL;
      ...
      if (!alive) {
        ...
        if (inst->config.reconnect_enabled && !inst->stopRequested) {
          ULONGLONG delay =
              calc_reconnect_delay(&inst->config, inst->reconnectAttempts);
  ...
  for (unsigned int i = 0; i < oldCount; i++) {
    ...
    for (unsigned int j = 0; j < newCfg->instance_count; j++) {
      ...
      if (oldInst->config.display_monitor_count != nc->display_monitor_count ||
          ...
          oldInst->config.display_monitor_height !=
              nc->display_monitor_height ||
      ...
      if (oldInst->config.codec_nscodec != nc->codec_nscodec ||
          ...
          oldInst->config.codec_frame_acknowledge !=
              nc->codec_frame_acknowledge) {
        is_breaking = 1;
      }
  ...
  for (unsigned int i = 0; i < newCfg->instance_count; i++) {
    ...
    for (unsigned int j = 0; j < oldCount; j++) {
      if (strcmp(oldInstances[j].name, nc->name) == 0) {
        ...
        if (strcmp(oldInst->config.backend_hostname, nc->backend_hostname) !=
                0 ||
            oldInst->config.backend_port != nc->backend_port ||
            strcmp(oldInst->config.backend_username, nc->backend_username) !=
                0 ||
            strcmp(oldInst->config.backend_domain, nc->backend_domain) != 0) {
          is_breaking = 1;
        }
        ...
        if (oldInst->config.display_monitor_count !=
                nc->display_monitor_count ||
            oldInst->config.display_monitor_width !=
                nc->display_monitor_width ||
            oldInst->config.display_monitor_height !=
                nc->display_monitor_height ||
            oldInst->config.display_color_depth != nc->display_color_depth) {
          is_breaking = 1;
        ...
        if (strcmp(oldInst->config.backend_password, nc->backend_password) !=
            0) {
          is_breaking = 1;
        }
