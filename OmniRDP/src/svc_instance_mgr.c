 ...
 */
static void build_heartbeat_pipe_name(const char *instanceName,
 ...
 */
static PipeInstanceState map_state_to_pipe(InstanceState s) {
    switch (s) {
        case INST_STOPPED:      return INSTANCE_STOPPED;
        case INST_STARTING:     return INSTANCE_STARTING;
        case INST_RUNNING:      return INSTANCE_RUNNING;
        case INST_RECONNECTING: return INSTANCE_RECONNECTING;
        default:                return INSTANCE_STOPPED;
    }
}
 ...
 */
static ULONGLONG calc_reconnect_delay(const InstanceConfig *cfg,
...
int inst_mgr_init(InstanceManager *mgr, SvcConfig *config,
    ...
    mgr->instances = (ManagedInstance *)calloc(
        config->instance_count, sizeof(ManagedInstance));
...
int inst_mgr_start(InstanceManager *mgr, const char *instanceName) {
    ...
    if (mgr->shuttingDown) {
        LOG_W("svc_inst_mgr", "Cannot start instance '%s': service is shutting down",
              instanceName);
        return -1;
    }
    ...
    if (inst->state != INST_STOPPED) {
        LOG_W("svc_inst_mgr", "Start: instance '%s' already running (state=%d)",
              instanceName, (int)inst->state);
        LeaveCriticalSection(&mgr->lock);
        return -1;
    }
    ...
    if (svc_dpapi_is_encrypted(inst->config.backend_password)) {
        ...
        if (svc_dpapi_decrypt(inst->config.backend_password,
                              decrypted_password,
                              sizeof(decrypted_password)) != 0) {
            LOG_E("svc_inst_mgr", "Start: failed to decrypt password for '%s'",
                  instanceName);
            inst->state = INST_STOPPED;
            LeaveCriticalSection(&mgr->lock);
            return -1;
        ...
        if (svc_dpapi_encrypt_in_file(mgr->configPath, instanceName,
                                       "backend.password") != 0) {
            LOG_W("svc_inst_mgr", "Start: failed to encrypt password in config "
                  "file for '%s' (continuing)", instanceName);
        }
        ...
        if (svc_dpapi_encrypt(inst->config.backend_password,
                              encrypted_buf, sizeof(encrypted_buf)) != 0) {
            LOG_E("svc_inst_mgr", "Start: failed to encrypt password for '%s'",
                  instanceName);
            inst->state = INST_STOPPED;
            LeaveCriticalSection(&mgr->lock);
            return -1;
        ...
        if (svc_dpapi_decrypt(encrypted_buf, decrypted_password,
                              sizeof(decrypted_password)) != 0) {
            LOG_E("svc_inst_mgr", "Start: failed to re-decrypt password for '%s'",
                  instanceName);
            SecureZeroMemory(encrypted_buf, sizeof(encrypted_buf));
            inst->state = INST_STOPPED;
            LeaveCriticalSection(&mgr->lock);
    ...
    if (OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &hToken)) {
        ...
        if (pTokenUser && GetTokenInformation(hToken, TokenUser, pTokenUser, tokenInfoLen, &tokenInfoLen)) {
    ...
    /* Fallback: if anything failed, use NULL descriptor (inherits process default) */
    ...
    if (!SetHandleInformation(hPipeRead, HANDLE_FLAG_INHERIT,
                              HANDLE_FLAG_INHERIT)) {
        LOG_W("svc_inst_mgr", "Start: SetHandleInformation read end failed "
              "for '%s' (err=%lu)", instanceName, GetLastError());
    }

    /* ── Build command line ────────────────────────────────────── */
    /*
     * Format:
     *   "<exePath>" --instance "<name>" --secrets-handle <handle> --config "<configPath>"
     */
    char cmdline[32768];
    ...
    /* ── Create heartbeat named pipe for this instance ─────────── */
    {
        ...
        inst->hHeartbeatPipe = CreateNamedPipeA(
            heartbeatPipeName,
            PIPE_ACCESS_INBOUND | FILE_FLAG_OVERLAPPED,
            PIPE_TYPE_BYTE | PIPE_READMODE_BYTE | PIPE_WAIT,
            1, 0, 65536, 0, NULL);
        if (inst->hHeartbeatPipe == INVALID_HANDLE_VALUE) {
            LOG_W("inst_mgr", "Failed to create heartbeat pipe for '%s' (error %lu)",
                  instanceName, GetLastError());
            inst->hHeartbeatPipe = NULL;
        }
    }
    ...
    if (!SetInformationJobObject(hJob, JobObjectExtendedLimitInformation,
        ...
        LOG_W("svc_inst_mgr", "Start: SetInformationJobObject failed for '%s' "
              "(err=%lu)", instanceName, GetLastError());
    ...
    if (!AssignProcessToJobObject(hJob, pi.hProcess)) {
        LOG_W("svc_inst_mgr", "Start: AssignProcessToJobObject failed for '%s' "
              "(err=%lu)", instanceName, GetLastError());
    ...
    if (!WriteFile(hPipeWrite, decrypted_password, (DWORD)pwd_len,
                   &written, NULL)) {
        LOG_E("svc_inst_mgr", "Start: WriteFile to secret pipe failed for '%s' "
              "(err=%lu)", instanceName, GetLastError());
    ...
    LOG_I("svc_inst_mgr", "Started instance '%s' (PID=%lu, job=%p)",
          instanceName, inst->pid, (void*)inst->hJob);
...
int inst_mgr_stop(InstanceManager *mgr, const char *instanceName) {
    ...
    if (inst->state == INST_STOPPED) {
        LOG_D("svc_inst_mgr", "Stop: instance '%s' already stopped",
              instanceName);
        LeaveCriticalSection(&mgr->lock);
        return 0;
    }
    ...
    if (!GenerateConsoleCtrlEvent(CTRL_BREAK_EVENT, pid)) {
        LOG_W("svc_inst_mgr", "Stop: GenerateConsoleCtrlEvent failed for '%s' "
              "(err=%lu)", name_copy, GetLastError());
    }
    ...
    if (waitResult == WAIT_TIMEOUT) {
        LOG_W("svc_inst_mgr", "Stop: instance '%s' did not exit within %u sec, "
              "terminating", name_copy, mgr->config->service.graceful_shutdown_sec);
        ...
        LOG_E("svc_inst_mgr", "Stop: unexpected WaitForSingleObject result for "
              "'%s' (%lu)", name_copy, GetLastError());
    }
...
int inst_mgr_restart(InstanceManager *mgr, const char *instanceName) {
    ...
    if (inst_mgr_stop(mgr, instanceName) != 0) {
        LOG_W("svc_inst_mgr", "Restart: stop failed for '%s', attempting start "
              "anyway", instanceName);
    }
...
void inst_mgr_start_all(InstanceManager *mgr) {
    ...
    for (unsigned int i = 0; i < mgr->instanceCount; i++) {
        if (!mgr->instances[i].config.enabled) {
            LOG_D("svc_inst_mgr", "Skipping disabled instance '%s'",
                  mgr->instances[i].name);
            continue;
        }

        if (inst_mgr_start(mgr, mgr->instances[i].name) != 0) {
            LOG_E("svc_inst_mgr", "Failed to start instance '%s'",
                  mgr->instances[i].name);
        }

        /* Delay between spawns if configured and not the last instance */
        if (mgr->config->service.instance_startup_delay_ms > 0 &&
            i + 1 < mgr->instanceCount) {
    ...
    for (unsigned int i = 0; i < mgr->instanceCount; i++) {
        ...
        if (inst->state == INST_RUNNING && inst->hProcess != NULL) {
            ...
            if (!GetExitCodeProcess(inst->hProcess, &exitCode)) {
                LOG_W("svc_inst_mgr", "Poll: GetExitCodeProcess failed for '%s' "
                      "(err=%lu)", inst->name, GetLastError());
            } else if (exitCode != STILL_ACTIVE) {
                alive = FALSE;
                LOG_W("svc_inst_mgr", "Poll: instance '%s' exited (code=%lu)",
                      inst->name, exitCode);
            }
            ...
            ULONGLONG timeoutMs = (ULONGLONG)
                mgr->config->service.heartbeat_timeout_sec * 1000ULL;
            if (inst->lastHeartbeatMs > 0 &&
                (nowMs - inst->lastHeartbeatMs) > timeoutMs) {
                LOG_W("svc_inst_mgr", "Poll: heartbeat timeout for '%s' "
                ...
                if (alive && inst->hProcess != NULL) {
                    ...
                    LOG_W("svc_inst_mgr", "Poll: force-killed '%s' due to "
                          "heartbeat timeout", inst->name);
                }
                alive = FALSE;
            }
#endif
            ...
            if (!alive) {
                ...
                if (inst->config.reconnect_enabled && !inst->stopRequested) {
                    ULONGLONG delay = calc_reconnect_delay(
                        &inst->config, inst->reconnectAttempts);
                    inst->nextReconnectMs = nowMs + delay;
                    inst->state = INST_RECONNECTING;
                    LOG_I("svc_inst_mgr", "Poll: instance '%s' will reconnect in "
                          "%llu ms (attempt %u)", inst->name, delay,
                          inst->reconnectAttempts + 1);
                } else {
                    inst->state = INST_STOPPED;
                    inst->reconnectAttempts = 0;
                    LOG_I("svc_inst_mgr", "Poll: instance '%s' stopped "
                          "(reconnect disabled or stop requested)", inst->name);
                }
            }
        }
        ...
        if (inst->state == INST_RECONNECTING) {
            if (nowMs >= inst->nextReconnectMs) {
                /* Check max attempts */
                if (inst->reconnectAttempts >=
                    inst->config.reconnect_max_attempts) {
                    LOG_W("svc_inst_mgr", "Poll: instance '%s' reached max "
                ...
                if (rc == 0) {
                    LOG_I("svc_inst_mgr", "Poll: instance '%s' reconnected "
                    ...
                    LOG_W("svc_inst_mgr", "Poll: reconnect attempt %u failed for "
                          "'%s', next in %llu ms",
...
int inst_mgr_reload_config(InstanceManager *mgr, const char *configPath) {
    ...
    if (!newCfg) {
        LOG_E("svc_inst_mgr", "Reload: failed to load config from '%s'",
              configPath);
        return -1;
    }
    ...
    for (unsigned int i = 0; i < newCfg->instance_count; i++) {
        ...
        if (inst->backend_hostname[0] == '\0') {
            LOG_E("svc_inst_mgr", "Reload: instance '%s' missing backend.hostname",
                  inst->name);
            svc_config_free(newCfg);
            return -1;
        }
        if (inst->backend_username[0] == '\0') {
            LOG_E("svc_inst_mgr", "Reload: instance '%s' missing backend.username",
                  inst->name);
            svc_config_free(newCfg);
            return -1;
        }
        if (inst->backend_port == 0) {
            LOG_E("svc_inst_mgr", "Reload: instance '%s' missing backend.port",
                  inst->name);
            svc_config_free(newCfg);
            return -1;
        }
        if (inst->viewer_port == 0) {
            LOG_E("svc_inst_mgr", "Reload: instance '%s' missing viewer.port",
                  inst->name);
            svc_config_free(newCfg);
            return -1;
        }
        ...
        for (unsigned int j = i + 1; j < newCfg->instance_count; j++) {
            if (newCfg->instances[j].viewer_port == inst->viewer_port) {
                LOG_E("svc_inst_mgr", "Reload: duplicate viewer.port %u between "
                      "'%s' and '%s'", inst->viewer_port, inst->name,
                      newCfg->instances[j].name);
                svc_config_free(newCfg);
                return -1;
            }
            if (newCfg->instances[j].backend_port == inst->backend_port &&
                strcmp(newCfg->instances[j].backend_hostname,
                       inst->backend_hostname) == 0) {
                LOG_W("svc_inst_mgr", "Reload: duplicate backend %s:%u between "
                      "'%s' and '%s'", inst->backend_hostname, inst->backend_port,
                      inst->name, newCfg->instances[j].name);
    ...
    for (unsigned int i = 0; i < oldCount; i++) {
        ...
        if (!found && oldInst->state != INST_STOPPED) {
            LOG_I("svc_inst_mgr", "Reload: instance '%s' removed from config, "
                  "stopping", oldInst->name);
    ...
    for (unsigned int i = 0; i < oldCount; i++) {
        ...
        for (unsigned int j = 0; j < newCfg->instance_count; j++) {
            ...
            if (oldInst->config.enabled && !nc->enabled &&
                oldInst->state != INST_STOPPED) {
                LOG_I("svc_inst_mgr", "Reload: instance '%s' disabled, stopping",
                      oldInst->name);
                inst_mgr_stop(mgr, oldInst->name);
            }
            break;
    ...
    for (unsigned int i = 0; i < oldCount; i++) {
        ...
        for (unsigned int j = 0; j < newCfg->instance_count; j++) {
            ...
            /* Viewer bind/port */
            if (strcmp(oldInst->config.viewer_bind_address,
                       nc->viewer_bind_address) != 0 ||
            ...
            if (oldInst->config.security_tls_enabled    != nc->security_tls_enabled ||
                ...
                oldInst->config.security_ignore_certificate !=
                    nc->security_ignore_certificate) {
                is_breaking = 1;
            }
            ...
            if (is_breaking) {
                LOG_I("svc_inst_mgr", "Reload: breaking change for '%s', "
                      "stopping for restart", nc->name);
    ...
    for (unsigned int i = 0; i < newCfg->instance_count; i++) {
        ...
        for (unsigned int j = 0; j < oldCount; j++) {
            if (strcmp(oldInstances[j].name, nc->name) == 0) {
                ...
                if (!oldInstances[j].config.enabled) {
                    LOG_I("svc_inst_mgr", "Reload: instance '%s' was disabled, "
                          "now enabled – starting", nc->name);
                    should_start = 1;
                    break;
                }
                ...
                if (is_breaking) {
                    LOG_I("svc_inst_mgr", "Reload: restarting instance '%s' after "
                          "breaking change", nc->name);
                    should_start = 1;
                }
                break;
            }
        ...
        if (should_start) {
            if (inst_mgr_start(mgr, nc->name) != 0) {
                LOG_E("svc_inst_mgr", "Reload: failed to start instance '%s'",
                      nc->name);
            }
        }
    }
    ...
    LOG_I("svc_inst_mgr", "Config reloaded successfully (%u instances)",
          newCfg->instance_count);
