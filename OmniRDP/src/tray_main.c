...
static int
install_autostart(void)
{
	HKEY hKey;
	LONG err = RegOpenKeyExA(HKEY_LOCAL_MACHINE, RUN_KEY_PATH, 0, KEY_SET_VALUE, &hKey);
...
WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (lpCmdLine && (strstr(lpCmdLine, "--help") != NULL || strstr(lpCmdLine, "-h") != NULL))
	{
	...
	INITCOMMONCONTROLSEX icc = { .dwSize = sizeof(icc), .dwICC = ICC_LISTVIEW_CLASSES };
	...
	if (g_ctx.serviceCount > 0)
	{
		...
		if (!anyInstances)
		{
			...
			if (result == IDYES)
			{
				...
				snprintf(configPath, sizeof(configPath), "C:\\ProgramData\\OmniRDP\\config.ini");
				...
				if (attr == INVALID_FILE_ATTRIBUTES)
				{
					...
					if (fp)
					{
						fprintf(fp, "; OmniRDP Configuration\n"
